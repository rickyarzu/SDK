...
uses System.IOUtils, AsyncTask, Janua.FMX.Resources, Janua.Core.Functions, udmErofloraAppLocalData, Janua.FMX.Functions;
...
procedure TfrmEuroFloraAppMain.FormKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
  if Key = vkHardwareBack then
  begin
    if (TabControlMain.ActiveTab = self.tab1Fair) and (TabControl1Fair.ActiveTab = self.tabItem12FairList) then
    begin
...
begin
  self.JanuaMapListFair.Coordinates := dmEurofloraAppLocalData.Coordinates;
end;
...
begin
  ...
  begin
    ...
    if (dmEurofloraAppLocalData.Coordinates.Latitude <> 0) and (dmEurofloraAppLocalData.Coordinates.Longitude <> 0) then
...
begin
  ...
  begin
    ...
    FMe := Janua.FMX.Functions.AddMarker(self.FMapViewAroundDetail, Coord, JanuaMapListAround.SelectedItem.LastName);
