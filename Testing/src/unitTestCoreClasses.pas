...
uses
  DUnitX.TestFramework, Janua.Core.Classes, Janua.Test.FireDac.dmVirtual;
...
type
  ...
  private
    FdmTestFunctionsDatase: TdmFDacVirtualTest;
...
begin
  aComponent := TJanuaCoreComponent.Create(nil);
  aComponent.Activate;
...
{$ENDIF HAS_VCL}
  FdmTestFunctionsDatase := TdmFDacVirtualTest.Create(nil);
  obj := TObj.Create;
...
begin
  (*
    ...
    lBindableControl.BindManager.BindCount.ToString); Assert.IsTrue(lBindableControl.BindManager.HasBindings,
    'HasBindings False');
    ...
    {$IFDEF HAS_VCL}
    var lRecordO, lRecordE: IJanuaBindRecord; lControl: IJanuaBindableEditControl;

    begin
    // aForm.Edit1.OnChange := obj.onEditChange;
    aForm.Edit1.Text := 'Ciao'; aBindableObject.TestString := 'Belin';
    ...
    lRecordO.Bind('TestString', aForm.Edit1, 'Text'); Assert.AreEqual(1, aBindableObject.BindManager.BindCount);
    ...
    // ed eseguo il bind che testo
    lRecordE.Bind('Text', aBindableObject, 'TestString'); Assert.AreEqual(1, lControl.BindManager.BindCount,
    'aForm.Edit1'); Assert.AreEqual(1, aBindableObject.BindManager.BindCount);

    Assert.AreEqual('Belin', aBindableObject.TestString, 'TestString <> Belin');
    Assert.AreEqual(aBindableObject.TestString, aForm.Edit1.Text);
    // testo il bind 'inverso'
    aForm.Edit1.Text := 'Ciao'; Assert.AreEqual(aForm.Edit1.Text, aBindableObject.TestString,
    'Test Form->Oggetto');

    lRecordO.ClearBindings; lRecordO := nil; Assert.AreEqual(0, aBindableObject.BindManager.BindCount,
    'aBindableObject lRecordO ClearBind'); Assert.AreEqual(1, lControl.BindManager.BindCount,
    'aForm.Edit1 lRecord0');

    lRecordE.ClearBindings; lRecordE := nil; Assert.AreEqual(0, aBindableObject.BindManager.BindCount,
    'aBindableObject lRecordE ClearBind'); Assert.AreEqual(0, lControl.BindManager.BindCount,
    'aForm.Edit1 lRecordE');
  *)
end;
...
begin
  ...
  begin
    FTestString := Value;
    BindManager.Notify('TestString');
    // does notify that TestString changed
  end;
end;
