...
{$IFDEF TEST_VCL}
  ...
  Janua.Test.VCLApplication, ufrmTest,
{$ENDIF TEST_VCL}
  System.SysUtils, System.Rtti, Spring, Spring.Collections, Janua.Orm.Intf, System.JSON, System.DateUtils,
  Janua.Application.Framework, Janua.Orm.Test.Intf, Janua.Server.Intf, Janua.Orm.Impl,
  Janua.Core.Types, Janua.Server.Impl, Janua.Orm.Test.Impl, Janua.Mocks.Helpers, Janua.Orm.Register;
...
{$IFDEF TEST_VCL}lFormTest: TfrmTest; {$ENDIF TEST_VCL}
begin
  ...
  // Aggiunngo un Record e lo valorizzo
  aRecordSet.Append;
...
{$IFDEF TEST_VCL}
  lFormTest := TfrmTest.Create(nil);
  try
    aRecordSet.TestInteger.Bind('AsInteger', lFormTest.SpinEdit1, 'Value');
    Assert.AreEqual(aRecordSet.TestInteger.AsInteger, lFormTest.SpinEdit1.Value);
    aRecordSet.TestInteger.AsInteger := 200;
    Assert.IsTrue(1 = aRecordSet.TestInteger.BindManager.BindCount,
      'Count = ' + aRecordSet.TestInteger.BindManager.BindCount.ToString);
    Assert.AreEqual(aRecordSet.TestInteger.AsInteger, lFormTest.SpinEdit1.Value);
    Assert.IsTrue(1 = lFormTest.SpinEdit1.BindManager.BindCount,
      'Count = ' + aRecordSet.TestInteger.BindManager.BindCount.ToString);
    lFormTest.SpinEdit1.Value := 3000;
    Assert.AreEqual(aRecordSet.TestInteger.AsInteger, lFormTest.SpinEdit1.Value, 'TestInteger->SpinEdit1');
  finally
    lFormTest.Free;
  end;
{$ENDIF TEST_VCL}
...
procedure TOrmTest.FilterContainerList;
var
  { The Field is a member of a Parameters' List }
  lParams: IJanuaParams;
  { This is the Field where the Container Will be Attached }
  lParam: IJanuaField;
  { Test Strings for Parameter's Filter Text }
  lTmp, lExpected: string;
  vTest: Boolean;
  { The Container To be Test }
  lContainer: IJanuaSearchListContainer;
begin
  lParams := TJanuaParams.Create;
  { Add Param Creates a Param Field named fint pointing to dbfield fint with default Value 0 not Active }
  lParam := lParams.AddParam('fint', TJanuaFieldType.jptInteger, 0);
  Assert.IsNotNull(lParam, 'lParam');
  { Default Value should not be necessary }
  lParam.DefaultValue := 0;
  { Standard Opertor Equals: = }
  lParam.ParamOperator := TJanuaOperator.Equals;
  { According to List Container Interface will try to Get the Container and have it Attached to the Field }
  // TJanuaApplicationFactory.RegisterClass(IJanuaSearchListContainer, TJanuaSearchListContainer);
  // TryGetContainer(const IID: TGUID; const aGroupID: Variant; aField: IJanuaField; out Intf): boolean;
  vTest := TJanuaOrmFactory.TryGetContainer(IJanuaSearchListContainer, -1, lParam, lContainer);
  Assert.IsTrue(vTest, 'IJanuaSearchListContainer not set');
  Assert.IsNotNull(lContainer, 'lContainer');

  { procedure AddKeyValue(const aKey, aValue: string); overload;
    procedure AddKeyValue(const aKeyValue: TKeyValue); overload; }
  lContainer.AddKeyValue('0', 'None');
  lContainer.AddKeyValue('1', 'One');
  lContainer.AddKeyValue('2', 'Two');
  Assert.AreEqual(3, lContainer.KeyCount);

  lContainer.KeyValue := '1';
  Assert.AreEqual('1', lContainer.KeyValue);
  { The final Test is to check that Selected Text on Container has set the correct property Value to Field }
  Assert.AreEqual(1, lParam.AsInteger);
  { And, eventually, the right corresponding Text Value for the Screen to show }
  Assert.AreEqual('One', lContainer.LookupValue);
end;
...
procedure TOrmTest.ForInLoop;
var
  aRecord, bRecord, lRecord: IJanuaRecord;
...
begin
  ...
  j := 0;
  i := 0;
  for lRecord in aRecordSet do
  begin
    Inc(i);
    t := lRecord.FieldByName('int').AsInteger;
    Assert.AreEqual(100, t, 't = ' + i.ToString);
    Inc(j, lRecord.FieldByName('int').AsInteger);
  end;

  Assert.AreEqual(3, i, 'lRecordSet Iterator i and 3 loops are not equal');
  Assert.AreEqual(300, j, 'lRecordSet Iterator j and 300 are not equal');
...
begin
  aRecord := Janua.Orm.Impl.TJanuaOrmFactory.CreateRecord('test');
  Assert.AreEqual('test', aRecord.Name);
...
begin
  ...
  *)
  ...
  for i := 0 to Pred(LOrigin.Count) do
    Assert.IsTrue(SameValue(LOrigin.Items[i].Value, LDestination.Items[i].Value),
      'LOrigin: ' + LOrigin.Items[i].Key + ' = ' + LOrigin.Items[i].AsString + ' Item <> LDestination: ' +
      LDestination.Items[i].Key + ' = ' + LDestination.Items[i].AsString);
...
begin
  ...
  Assert.AreEqual(lParam.AsString, 'BELIN');
  Assert.IsTrue(lParam.Active, 'lParam(jptString).Active');

  lParam := lParams.AddParam('campo2', TJanuaFieldType.jptInteger, 100);
  lParam.ParamOperator := TJanuaOperator.LessThan;
  { lParam.Activate; }

  Assert.AreEqual(lParam.AsInteger, 100);
  Assert.IsTrue(lParam.Active, 'lParam(jptInteger).Active');

  lParams.Activate;
  Assert.IsTrue(lParams.Active, 'lParams');
