unit ufrmJanuaCoreTestingDebug;
...
uses udmTestFunctionsDataset, Janua.Maps.Types, Janua.core.Maps, Janua.Maps.Intf;
...
begin
  ...
  try
    aFrom.AssignDataset(dmTestFunctionsDataset.VirtualTable1);
    self.DataSource2.DataSet := aFrom.MemDataset;
    aJson := aFrom.AsJson;
    self.Label1.Caption := aJson;
    aTo := TJanuaJsonDacDataset.Create(nil);
    try
      aTo.LoadFromJson(aJson);
      self.DataSource3.DataSet := aTo.MemDataset;
