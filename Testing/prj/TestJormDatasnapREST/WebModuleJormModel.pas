...
type
  TWebModule1 = class(TWebModule)
    ...
    procedure ServerFunctionInvokerHTMLTag(Sender: TObject; Tag: TTag; const TagString: string;
...
uses ServerMethodsJormCms, Web.WebReq;

procedure TWebModule1.DSServerClass1GetClass(DSServerClass: TDSServerClass;
  var PersistentClass: TPersistentClass);
begin
  PersistentClass := ServerMethodsJormCms.TServerMethods1;
end;
...
begin
  if SameText(TagString, 'urlpath') then
    ...
    ReplaceText := ServerMethodsJormCms.TServerMethods1.ClassName
...
begin
  ...
  if SameFileName(ExtractFileName(AFileName), 'serverfunctions.js') then
    if not FileExists(AFileName) or (FileAge(AFileName, D1) and FileAge(WebApplicationFileName, D2) and
      (D1 < D2)) then
    begin
