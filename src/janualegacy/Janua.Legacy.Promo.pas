unit Janua.Legacy.Promo;

interface

uses
  Classes, // Januaproject
  Janua.Legacy.Core;

type
  /// <author>Riccardo Arduino</author>
  TJanuaLegacyPromo = class(TComponent)
  private
    FTextMessage: string;
  ...
  published
    ...
    Property DiscountMoney: Double read FDiscountMoney write SetDiscountMoney; // socnto unitario in euro
...
uses SysUtils, Janua.Core.Functions;

{ TPromo }

procedure TJanuaLegacyPromo.PromoCalculation;
begin
  FToday := Date();
  ...
  FOutputMessage := StringReplace(FTextMessage, '<DISCOUNT>', FloatToStr(FDiscountMoney),
    [rfReplaceAll, rfIgnoreCase]);
  FOutputMessage := StringReplace(FOutputMessage, '<MINIMUMEXPENSE>', FloatToStr(FMinimumExpenseRequired),
    [rfReplaceAll, rfIgnoreCase]);
  FOutputMessage := StringReplace(FOutputMessage, '<VALIDFROM>', DateToStr(FValidFrom),
    [rfReplaceAll, rfIgnoreCase]);
  FOutputMessage := StringReplace(FOutputMessage, '<VALIDTO>', DateToStr(ValidTo),
    [rfReplaceAll, rfIgnoreCase]);

  FPromoOutputMessage := StringReplace(FPromoMessage, '<DISCOUNT>', FloatToStr(FDiscountApplied),
    [rfReplaceAll, rfIgnoreCase]);
  FPromoOutputMessage := StringReplace(FPromoOutputMessage, '<MINIMUMEXPENSE>',
    FloatToStr(FMinimumExpenseRequired), [rfReplaceAll, rfIgnoreCase]);
  FPromoOutputMessage := StringReplace(FPromoOutputMessage, '<EXPENSE>', FloatToStr(FAmountExpense),
    [rfReplaceAll, rfIgnoreCase]);
end;

procedure TJanuaLegacyPromo.SetAmountExpense(const Value: Double);
begin
  FAmountExpense := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetDiscountMoney(const Value: Double);
begin
  FDiscountMoney := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetDiscountPurcent(const Value: Double);
begin
  FDiscountPurcent := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetMinimumExpenseRequired(const Value: Double);
begin
  FMinimumExpenseRequired := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetPromoID(const Value: int64);
begin
  FPromoID := Value;
end;

procedure TJanuaLegacyPromo.SetPromoIDApplied(const Value: int64);
begin
  FPromoIDApplied := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetPromoItemID(const Value: int64);
begin
  FPromoItemID := Value;
end;

procedure TJanuaLegacyPromo.SetPromoMessage(const Value: string);
begin
  FPromoMessage := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetTextMessage(const Value: string);
begin
  FTextMessage := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetValidFrom(const Value: TDateTime);
begin
  FValidFrom := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.SetValidTo(const Value: TDateTime);
begin
  FValidTo := Value;
  PromoCalculation
end;

procedure TJanuaLegacyPromo.ShowMessagePromo;
begin
  if FDiscountEnabled then
