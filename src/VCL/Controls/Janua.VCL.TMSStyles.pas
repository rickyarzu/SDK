...
uses
  ...
  AdvGlowButton, Janua.Core.System, System.TypInfo, VCL.Menus, VCL.Forms,
  AdvPanel, AdvOfficeTabSet, AdvToolBar;

type
  TJanuaTMSStyle = record
    Style: TTMSStyle;
    StyleName: string;
...
type
  ...
  private
    FStyle: TTMSStyle;
    arrayStyles: array of TJanuaTMSStyle;
    FPopUpMenu: TAdvPopupMenu;
    FButton: TAdvGlowMenuButton;
    FMetroStyle: boolean;
    FJanuaStyle: TJanuaTMSStyle;
  ...
  protected
    ...
    procedure SeTJanuaTMSStyle(const Value: TJanuaTMSStyle);
  strict protected
  ...
  published
    ...
    property JanuaStyle: TJanuaTMSStyle read FJanuaStyle;
  end;
...
Type
  ...
  public
    ...
    property JanuaStyle: TJanuaTMSStyle read FJanuaStyle write SeTJanuaTMSStyle;
    destructor Destroy; override;
    constructor Create(AOwner: TComponent); override;
  published
    property JanuaStyleManager: TJanuaTMSStyleManager read FJanuaStyleManager write SeTJanuaTMSStyleManager;
...
begin
  self.SeTJanuaTMSStyle(self.arrayStyles[TMenuItem(Sender).Tag]);
...
begin
  if not(csDesigning in ComponentState) and self.Active then
  begin
    SeTJanuaTMSStyle(arrayStyles[TJanuaCoreOS.ReadParam('Application', 'Style', Ord(TTMSStyle.tsOffice2003Blue))]);
  end;
end;
...
begin
  ...
  if assigned(self.FAppStyler) then
    self.FAppStyler.Style := FJanuaStyle.Style
end;
...
procedure TJanuaTMSStyleManager.SeTJanuaTMSStyle(const Value: TJanuaTMSStyle);
begin
  FJanuaStyle := Value;
  TJanuaCoreOS.WriteParam('Application', 'Style', Ord(self.FJanuaStyle.Style));
...
begin
  ...
  begin
    ...
    begin
      self.SeTJanuaTMSStyle(arrayStyles[Ord(Value)]);
    end;
  end;
end;

function TJanuaTMSStyleManager.InternalActivate: boolean;
var
  aStyle: TTMSStyle;
  i: Integer;
...
function TJanuaFormStyler.InternalActivate: boolean;
begin
  Result := Active;
  if not Result then
    try
...
begin
  if assigned(FAdvFormStyler) and assigned(FJanuaStyleManager) and self.Active and not(csDesigning in ComponentState)
  then
  begin
    self.FAdvFormStyler.AppStyle := self.FJanuaStyleManager.AppStyler;
    SeTJanuaTMSStyle(FJanuaStyleManager.JanuaStyle);
  end;
end;

procedure TJanuaFormStyler.AdvFormStylerChange(Sender: TObject);
begin
  self.SeTJanuaTMSStyle(self.FJanuaStyleManager.JanuaStyle);
end;
...
procedure TJanuaFormStyler.SeTJanuaTMSStyle(const Value: TJanuaTMSStyle);
begin
...
procedure TJanuaFormStyler.SeTJanuaTMSStyleManager(const Value: TJanuaTMSStyleManager);
begin
