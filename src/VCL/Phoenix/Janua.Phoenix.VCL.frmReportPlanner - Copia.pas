...
type
  ...
  TfrmPhoenixVCLReportPlanner = class(TForm)
    ...
    procedure AdvWebBrowser1BeforeNavigate(Sender: TObject;
...
procedure TfrmPhoenixVCLReportPlanner.AdvWebBrowser1BeforeNavigate(Sender: TObject;
  var Params: TAdvCustomWebBrowserBeforeNavigateParams);
begin
  if (FCookies.Count > 0) and not FCookiesSet then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      var
      J := 0;
      while not vTest and (J < FCookies.Count) do
      begin
        vTest := (FCookies[J].Path = ACookies[I].Path) and (ACookies[I].Name = FCookies[J].Name);
        if vTest then
          FCookies.Replace(ACookies[I], J)
        else
          Inc(J);
      end;
      if not vTest then
...
procedure TfrmPhoenixVCLReportPlanner.AdvWebBrowser1Initialized(Sender: TObject);
begin
  { if FCookies.Count > 0 then
    for var I := 0 to FCookies.Count - 1 do
    AdvWebBrowser1.AddCookie(FCookies[I]); }
...
procedure TfrmPhoenixVCLReportPlanner.LoadCookies;
begin
  if (self.FCookies.Count = 0) then
