...
uses
  System.SysUtils, System.Classes, Data.DB, Datasnap.DBClient, Winapi.Windows, VCL.ComCtrls, Janua.Core.Backup,
  VCL.Dialogs;
...
begin
  ...
  begin
    ...
    begin
      ...
      Nodes[i] := i;
      if BNodes[i].FileDir.dir then
        BNodes[i].Node.ImageIndex := 0
      else
        BNodes[i].Node.ImageIndex := 5;
      if BNodes[i].FileDir.dir then
        BNodes[i].Node.SelectedIndex := 1
      else
        BNodes[i].Node.SelectedIndex := 6;
      self.FLocalDataset.Next;
    end;
    maxi := i;
    for l := mini to maxi do
    begin
      isdir := BNodes[l].FileDir.dir;
      dir := BNodes[l].FileDir.parentid;
      id := BNodes[l].FileDir.id;
      name := BNodes[l].FileDir.name;
      if BNodes[l].FileDir.dir then
        SetDir(l);
    end;
  end;
end;

procedure TJanuaVclBackupTreeMaker.SetDirectoryBackup(const Value: TJanuaDirectoryBackup);
begin
...
procedure TJanuaVclBackupTreeMaker.SetRemoteDataset(const Value: TClientDataset);
begin
