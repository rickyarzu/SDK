...
procedure Register;
begin
  RegisterComponents('januaproject', [TJanuaPromo]);
end;
...
begin
   ...
   FOutputMessage := StringReplace(FOutputMessage, '<MINIMUMEXPENSE>', FloatToStr(FMinimumExpenseRequired), [rfReplaceAll, rfIgnoreCase]);
   ...
   FPromoOutputMessage := StringReplace(FPromoMessage, '<DISCOUNT>', FloatToStr(FDiscountApplied), [rfReplaceAll, rfIgnoreCase]);
   FPromoOutputMessage := StringReplace(FPromoOutputMessage, '<MINIMUMEXPENSE>', FloatToStr(FMinimumExpenseRequired), [rfReplaceAll, rfIgnoreCase]);
   FPromoOutputMessage := StringReplace(FPromoOutputMessage, '<EXPENSE>', FloatToStr(FAmountExpense), [rfReplaceAll, rfIgnoreCase]);
end;
...
begin
  if FDiscountEnabled then JShowMessage(FPromoOutputMessage, '', '') else
