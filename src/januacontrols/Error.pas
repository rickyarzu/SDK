...
uses
  ...
  VCL.Forms, VCL.Dialogs, Data.DB, Menus, Grids, Printers, Janua.Core.System,
...
procedure Register;
begin
  RegisterComponents('Januaproject', [TExceptionHandler]);
end;
...
begin
       ...
       FContenuto :=
          'ERRORE DI ACCESSO ALLA MEMORIA : ' + #13 +
         ...
         '2. Verificare che altre postazioni di lavoro collegate in rete siano "ferme" nessun programma attivo' + #13 +
         '3. Riavviare il computer' + #13 +
         '4. Se possibile lanciare la procedura di verifica degli archivi "TABLESCANNER" appositamente studiata per questi casi' + #13 +
         '5. Entrare nel programma e verificare le funzionalità principali' + #13 +
...
begin
    ...
    ///
     ...
     begin
        ...
        if (pos('DOC_PASSANGER_LIST', STRINGA) > 0) and (pos  ('"ETA"', STRINGA) > 0)  then
            FRiga :=('Attenzione inserito un passeggero in polizza senza specificare età. (ORA-01400) Tabella:DOC_PASSANGER_LIST' + #13 +
            'premere Esc per annullare l''inserimento')
        else
        if (pos('ANAGRAFICA', STRINGA) > 0) and (pos  ('"COGNOME"', STRINGA) > 0)  then
            FRiga :=('Attenzione inserito cognome nullo o non valido. (ORA-01400) Tabella:ANAGRAFICA' + #13 +
            'premere Esc per annullare l''inserimento')
        else
        if (pos('ANAGRAFICA', STRINGA) > 0) and (pos  ('"ANAGRAFICA_ID"', STRINGA) > 0)  then
            FRiga :=('Attenzione inserita FRiga nulla in scheda anagrafica, contattare assistenza. (ORA-01400) Tabella:ANAGRAFICA' + #13 +
            'premere Esc per annullare l''inserimento')
        else
        if (pos('AN_PASSANGER_LIST', STRINGA) > 0) and (pos  ('"SESSO"', STRINGA) > 0)  then
            FRiga :=('Attenzione inserita FRiga in passanger List senza specificare il sesso. (ORA-01400) Tabella:ANAGRAFICA' + #13 +
            'premere Esc per annullare l''inserimento oppure compilare correttamente la colonna sesso')
        else
         FRiga :=('Attenzione, non è stato compilato un campo o non è stato completato un passaggio nell''inserimento del record.' + #13 +
                      'Provate a ripeter il passaggio che stavate eseguendo o premete il pulsante "Esc" sulla tastiera o il comando annulla del documento che stavate realizzando.');
     ...
     begin
        ...
        if pos('PLAN_FAMIGLIE_AREEES_PK', STRINGA) > 0 then
            FRiga :=('Attenzione, è stato immesso un valore duplicato nella tabella, forse avete inserito due volte la stessa famiglia.' + #13 +
            'premere Esc per annullare l''inserimento')
        else  if pos('COND_APPL_PK', STRINGA) > 0 then
            FRiga :=('Attenzione, è stato immesso un valore duplicato nella tabella, forse avete inserito due volte lo stesso termine di pagamento.' + #13 +
            'premere Esc per annullare l''inserimento')
        else  if pos('DOCPASS_PK', STRINGA) > 0 then
            FRiga :=('Attenzione, è stato immesso un nome duplicato in passanger list, omonimo e della stessa età.' + #13 +
            'premere Esc per annullare l''inserimento')
        else  if pos('NUOVA_TARGA', STRINGA) > 0 then
            FRiga :=('Attenzione, è stata immessa una targa omonima nella polizza. (codice: NUOVA_TARGA J001)' + #13 +
            'premere Esc per annullare l''inserimento')
            ...
            FRiga :=('Attenzione, è stato immesso un nome duplicato in passanger list, omonimo con stessa data di nascita.' + #13 +
            '(ORA-00001 ordini pass. list) premere Esc per annullare l''inserimento' )
            ...
            FRiga :=('Attenzione, è stato immesso uno sconto duplicato nella tabella degli sconti stagionali (ORA-00001 SCONTI PER LINEA E STAGIONE).'
            + #13  +
            'premere Esc per annullare l''inserimento')
        ...
        // da documenti....
         ...
         FRiga :=('Attenzione, è stata inserita una FRiga duplicata (ORA-00001), premere Esc per annullare l''inserimento'
                  );
         Result := True;
     end
     ...
     begin
         ...
         if pos('ACPDCATEG_ATE_FK', STRINGA) > 0 then
            FRiga :=(
            ' non si può cancellare questa famiglia perché possiede dei record collegati in catalogo.'
            )
            ...
            FRiga :=(
            'Attenzione, si è cercato di cancellare un prodotto già impiegato in ricevute/fatture'
            )
         ...
         FRiga :=('Attenzione si è cercato di cancellare una FRiga in una tabella dalla quale dipendono altri dati presenti nel programma.'
     ...
     begin
         FRiga :=('Attenzione state cercando modficare una polizza già contabilizzata.' + #13 +
                      ...
                      'Se è necessario effettuare una modifica potete richiederla in sede ...'
                      + #13 + 'Per contattare assistenza comunicare codice errore:  (ORA-20110 DOCUMENTI)'
                      );
         Result :=  True;
     end
     ...
     begin
         FRiga :=('Attenzione state cercando di realizzare un ritorno non valido.' + #13 +
                      ...
                      'Oppure esiste una prenotazione di ritorno chiusa ...'
                      + #13 + 'Per contattare assistenza comunicare codice errore:  (ORA-20112 SH_PKG)'
     ...
     begin
         FRiga :=('Attenzione state cercando di realizzare un ritorno non valido. (ORA-20111 SH_PKG)' + #13 +
                      ...
                      'Oppure esiste una polizza di ritorno stampata ...'
                      + #13 + 'Per contattare assistenza comunicare codice errore:  (ORA-20111 SH_PKG)'
                      );
         Result :=  True;
     end
     ...
     begin
         FRiga :=('Attenzione state cercando di ELIMINARE o annullare un documento contabilizzato.' + #13 +
                      ...
                      'Richiedere in ufficio lo sblocco del documento ...'
                      + #13 + 'Per contattare assistenza comunicare codice errore:  (ORA-20115 DOCUMENTI)'
                      );
         Result := True;
     end
     ...
     begin
         FRiga :=('Attenzione state cercando di modificare o cancellare una scrittura in prima nota.' + #13 +
                      ...
                      'Per poterla cambiare dovete riaprire il mese ...'
                      + #13 + 'Per contattare assistenza comunicare codice errore:  (ORA-20118 AC_PRIMANOTA)'
     ...
     begin
         FRiga :=('Attenzione Errore di Database o di Gestione Dati: ' + #13 +
                  stringa);
         Result :=  True;
     end;
...
begin
      FContenuto :=   'Istruzioni : ' + #13 +
           'Questo errore si verifica quando si cerca di inserire un codice o un nome già esistente nel database.' + #13 +
           'Se l''errore si verifica in seguito a una modifica o a un inserimento di un dato, nella maggiorparte dei casi conviene seguire i punti sottoelencati:' + #13 +
           '1. Verificare i campi chiave (di solito campi "Codice", "Targa",  "Matricola"), per rendersi conto se si è cercato di inserire un codice già presente' + #13 +
           '2. La verifica può essere fatta variando anche solo un carattere del campo chiave e provando a confermare la modifica con il segno di spunta presente nella pulsantiera.' + #13 +
           '3. Se il messaggio di errore persiste, provare ad annullare la modifica cliccando sul tasto "X" presente sulla pulsantiera (da non confondere con il tasto "Chiudi" che presenta una X rossa).' + #13 +
           '4. Se anche con questo sistema non si risolve il problema contattare il servizio assistenza che vi guiderà alla soluzione.'
           ;
...
begin
 ...
 If FileExists(FLogFileName) then
  ...
  FEsteso := 'ore: ' + DateTimeToStr(Now) +  #13 +
            'messaggio: ' +   e.message +  #13 +
 ...
 CloseFile(ErrFile) ;
   ...
   if E is EDivByZero then FRiga := ('Errore, divisione su zero')
   else if E is ERangeError then FRiga := ('Errore di range')
   else if E is EIntOverFlow then FRiga := ('Numero intero troppo lungo (di dimensioni eccessive).')
     { floating point math exceptions }
   else if E is EInvalidOp then FRiga := ('Operando non valido, errore nel puntatore.')
   else if E is EZeroDivide then FRiga := ('Errore, divisione per zero')
   else if E is EOverflow then FRiga := ('Errore, numero troppo lungo')
   else if E is EUnderflow then FRiga := ('EUnderflow')
   { eccezioni della RunTimeLibrary relative alla gestione Memoria }
   else if E is EOutOfMemory then FRiga := ('Memoria di sistema insufficiente')
   else if E is EOutOfResources then FRiga := ('Le risorse di sistema sono insufficienti, chiudete qualche applicazione')
   else if E is EInvalidPointer then FRiga := ('Tentativo di rilascio oggetto già liberato')
   ...
   else if E is EInvalidGraphicOperation then FRiga := ('Non è consentito modificare questo disegno')
   {Errori di conversione}
   else if E is EInvalidCast then FRiga := ('Errore di conversione')
   else if E is EConvertError then FRiga := ('Errore di conversione Dati')

   {Errori di tipo Hardware e dispositivi o periferiche}
    else if E is EPrinter then FRiga := ('Errore di comunicazione con la stampante')
    else if E is EAccessViolation then FRiga := ('Accesso errato alla memoria di sistema')
    else if E is EcontrolC then FRiga := ('Istruzione fermata. Utente con i tasti CTRL+C')
    else if E is EInOutError then FRiga := ('Errore di comunicazione con periferica esterna')

    {Errori nell'architettura OLE}
    else if E is EPropReadonly then FRiga := ('Accesso negato al programma OLE chiamato in Windows, sola lettura')
    else if E is EPropWriteOnly then FRiga := ('Accesso negato al programma OLE chiamato in Windows, sola Scrittura')
    {Errori nelle Classi VCL}
    else if E is EFCreateError then FRiga := ('Accesso negato al disco o alla rete, non riesco a creare il file')
    else if E is EFOpenError then FRiga := ('Accesso negato al disco o alla rete, non riesco ad aprire il file')
    else if E is EInvalidImage then FRiga := ('Accesso negato al disco o alla rete, non trovo il file di risorse, contattare il rivenditore')
    {Errori sui Menu}
    else if E is EMenuError then FRiga := ('Questa voce di menu non è ancora attivata o non punta a nulla')
    {Errori nelle griglie}
    else if E is EInvalidGridOperation then FRiga := ('Si è cercato di accedere a questa griglia in modo sbagliato o sono stati inseriti dati non corretti')
    else if E is EStringListError then FRiga := ('Problemi interni al sistema nella gestione delle liste, selezionato oggetto inesistente.')
    ...
    else if E is EOutOfMemory then FRiga := ('Esaurita la memoria di windows')
    else if E is EInvalidPointer then FRiga := ('Errore di accesso alla memoria, puntatore non valido.')

    {Errori di cui non conosco il tipo}
    else
     if (Copy(E.Message,1,20)  = 'Invalid input value.') then
   ...
   {****** Questa parte del programma cerca di analizzare gli errori generici di DB}
        ...
        begin
             ...
             if StringaErrore = 'Dataset not in edit or insert mode' then
                FRiga := ('Archivio non aggiornato in quanto non sono state rilevate modifiche errore del software: DATASET NOT IN EDIT OR INSERT MODE, contattare ASSISTENZA')
 ...
 if copy (e.message, 0,3) ='ORA' then risultato :=
                                SpiegaOracle(e.Message, FRiga, FEsteso);
 //*****************************************************************************

 // messaggio di violazione di chiave
 if e.message = 'Key violation.' then Risultato := SpiegaKey ;

 // errore di accesso alla memoria ....
 if pos('ACCESS',Uppercase(e.message)) > 0 then risultato :=  SpiegaAccess ;

 // errora di scrittura file su disco
 if e.message = 'Stream write error' then
 begin
     FRiga := ('Attenzione, impossibile scrivere su un file. Possibili cause: memoria o disco del computer pieni') ;
     risultato := True;
 end;
...
if (pos ('Field', e.Message) > 0) and (pos ('must have a value', e.Message) > 0) then
 begin
       ...
       begin
            FRiga := ('Attenzione si è cercato di inserire una nuova targa senza scrivere correttamente la targa stessa');
            risultato := True;
       end;
       if pos ('NASCITA_CITTA' , e.Message ) > 0 then
       begin
            FRiga := ('Attenzione si è cercato di inserire un passeggero in elenco senza indicare il luogo di nascita');
            risultato := True;
       end;
       if pos ('NASCITA_DATA' , e.Message ) > 0 then
       begin
          FRiga := ('Attenzione si è cercato di inserire un passeggero in elenco senza indicare la data di nascita');
          risultato := True;
       end;
       if pos ('TELEFONO' , e.Message ) > 0 then
       begin
          FRiga := ('Attenzione si è cercato di inserire una scheda priva del numero di Telefono, campo obbligatorio');
          risultato := True;
       end;
end;
...
begin
     ...
     begin
       FRiga := ('Attenzione state inserendo un valore in un campo percentuale, il valore che state scrivendo non può contenere il simbolo %. '
     ...
     begin
       FRiga := ('Attenzione state inserendo un valore in un campo numerico con virgola è ammesso l''inserimento di "," come separatore '
     ...
     begin
       FRiga := ('Attenzione, state srivendo su un campo numerico, avete inserito una lettera o un simbolo che non sono validi. '
...
begin
      FRiga := ('Attenzione, errore di gestione della memoria dati di windows, per continuare il vostro lavoro dovete chiudere e riavviare la console'
...
begin
      FRiga := ('Attenzione, il sistema di stampa non è al momento pronto o è occupato, nessuna stampante utilizzabile'
...
begin
     FRiga := ('Attenzione le risorse di Stampa sono momentaneamente impegnate, la stampa sarà ritardata di 10 secondi'
                  + #13 + 'Se il problema dovesse persistere contattate l''assistenza o TERMINATE il programma'
                  );
      risultato := True;
end;

  if not Risultato then FRiga := ('Si è verificato un errore:' + #13 + FRiga +
             #13 + 'Se non siete in grado ancora di gestire questo Contattare l''assistenza.');
...
begin
 ...
 try
   ...
   begin
    ...
    FEsteso := 'ore: ' + DateTimeToStr(Now) +  #13 +
                'messaggio: ' +   e.message +  #13 +
                'locazione: "' + Dataset.ClassName + '", ' + sLineBreak +
                'Componente: ' + Dataset.Name + sLineBreak;
    ...
    if not FSilent then ErrorMessageExtended;
   end;
...
procedure TExceptionHandler.SetJanuaOS(const Value: TJanuaCoreOS);
begin
  FJanuaOS := Value;
  if Assigned(FJanuaOS) then
    FLogFileName := FJanuaOS.GetDocumentDirectory + FJanuaOS.WildCard +
                    FJanuaOS.AppName + FJanuaOS.WildCard + 'Error.log'
  else
     self.FLogFileName := 'Error.log';
end;
