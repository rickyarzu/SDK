...
type
  ...
  protected
    ...
    function SetBind(const aServer: TJanuaCoreComponent; aTarget: TListBox): boolean;
    function SetBindObject(const aServer: TJanuaCoreObject; aTarget: TListBox): boolean;
  public
    { Public declarations }
...
function TJanuaVCLTestLog.Activate: boolean;
begin
  Result := inherited;
  self.WriteLog('TJanuaVCLTestLog.Activate');
  if Assigned(FJanuaServerSystem) and Assigned(FMemoSystem) and Assigned(FJanuaServerJPublic) and
    Assigned(FMemoJPublic) and Assigned(FJanuaProfile) and Assigned(FMemoProfile) then
  begin
    Result := self.SetBind(FJanuaServerSystem, FMemoSystem) and
      self.SetBind(FJanuaServerJPublic, FMemoJPublic) and self.SetBind(FJanuaProfile, FMemoProfile)
  end
  else
    Result := False;
  ...
  if Result then
  begin
    if Assigned(FMemoAddress) then
      self.SetBindObject(self.FJanuaProfile.Address, FMemoAddress);
  end;
end;
...
begin
  ...
  if Result then
    FJanuaServerSystem.Bind('Log', FMemoSystem, 'Items');
end;
...
function TJanuaVCLTestLog.SetBindObject(const aServer: TJanuaCoreObject; aTarget: TListBox): boolean;
begin
  Result := Assigned(aServer) and Assigned(aTarget);
  if Result then
    aServer.Bind('Log', aTarget, 'Items');
