...
type
  ...
  public
    ...
    procedure OpenSeasonsList; virtual; // OK
...
type
  ...
  public
    ...
    { TODO -cIndy : PreparePage(const aPage: string) hides virtual method }
    ...
    function PrepareChampionshipTeam(aUseFrame: boolean = False): boolean;
    // prepara la pagina Championship Team......
  public
...
type
  ...
  strict private
    ...
    FJanuaServerFootball: TJanuaCustomServerFootball;
    Notifications: TJanuaFootballNotifications;
    /// <summary>  array di notiifche all'utente puo essere connesso a srv push-stomp
    procedure SetJanuaServerFootball(const Value: TJanuaCustomServerFootball);
  ...
  public
    ...
    /// <summary> funzione di finalizzazione della prenotazione. Crea la Home Page.  </summary>
    function ActionHomePage(aUseFrame: boolean = False): boolean; //
    ...
    /// <summary>
    /// Genera la pagina del tabellino incontro, formato pagina o frame (Components involved):
...
type
  ...
  public
    ...
    function getChampionshipSummary: boolean; virtual; abstract;
    // (aSessionKey aSeasonID, aChampionshipID)
    // Server function getRanking(aSessionKey: string; aSeasonID, aChampionshipID: integer): TJsonObject;
    function getRanking: boolean; virtual; abstract;
    // (aSessionKey: aSeasonID, aChampionshipID: integer)
    // function getGoalRanking(aSessionKey: string; aSeasonID, aChampionshipID: integer): TJsonObject;
    function getGoalRanking: boolean; virtual; abstract;
    // (aSessionKey: aSeasonID, aChampionshipID: integer)
    // Server function getMatchDay(aSessionKey: string; aSeasonID, aChampionshipID, aMatchDay: integer): TJsonObject;
    function getMatchDay: boolean; virtual; abstract;
    // (aSessionKey: aSeasonID, aChampionshipID, aMatchDay: integer)
    // Server function getChampSummary(aSessionKey: string; aSeasonID, aChampionshipID: integer): TJsonObject;
    function getChampSummary: boolean; virtual; abstract;
    // (aSessionKey: aSeasonID, aChampionshipID, aMatchDay)
    // Server function getMatchDetails(aSessionKey: string; aSeasonID, aChampionshipID, aMatchID: integer): TJsonObject;
    function getMatchDetails: boolean; virtual; abstract;
    // (aSessionKey: aSeasonID, aChampionshipID, aMatchID)
    // Server function GetTeamChampionship(aSessionKey: string; aSeasonID, aChampionshipID, aTeamID: integer): TJsonObject;
    function GetTeamChampionship: boolean; virtual; abstract;
    // (aSessionKey: aSeasonID, aChampionshipID, aTeamID)
...
uses Janua.Core.Entities, Janua.Http.Types, System.Math, Janua.Core.Functions, System.StrUtils, Spring,
  Janua.Football.Impl, Janua.Application.Framework;
...
begin
  if notDesigning and Active and Assigned(self.FFootballSettings) and (season_id > 0) and (league_id > 0) then
...
begin
  Result := goalrankings.AsJsonPretty
end;
...
begin
  ...
  try
    ...
    if not aTransaction then
      self.EndTransaction;
    DestroyDataModule(not self.InTransaction and not self.KeepAlive);
  end;
end;
...
begin
  ...
  if not Result then
    try
      Result := inherited and notDesigning and not isInitializing and Assigned(FServerFootball) and
        FServerFootball.ActivateDatasets;
      if Result then
      begin
        SetInternalDatasources;
        Result := FServerFootball.OpenDefaultSeason;
      end;
    except
      on e: exception do
        RaiseException('TJanuaFootballRankingController.InternalActivate', e, self);
    end;
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('PrepareChampionshipTeam', e, self, self.LogText);
  end;
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('PrepareHome', e, self, self.LogText);
  end;
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('PrepareLiveScore', e, self, self.LogText);
  end;
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('PrepareMatchDetail', e, self, self.LogText);
  end;
end;
...
begin
  Result := False;
  { TODO -cFootball : Template Football Prepare Generic Page }
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('PreparePlayerCareer', e, self, self.LogText);
  end;
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('PrepareRanking', e, self, self.LogText);
  end;
end;
...
var
  f: TJanuaCustomServerFootball;
begin
  f := FJanuaServerFootball;
  self.TerminatePageContent;
  TerminateTemplatePage(aIsFrame);
...
begin
  ...
  with (FJanuaFootballController.JanuaServerFootball as TJanuaCustomServerFootball) do
  begin
...
try
  ...
  on e: exception do
    raise exception.Create('Janua.Core.Football.initialization ' + e.Message);
end;
