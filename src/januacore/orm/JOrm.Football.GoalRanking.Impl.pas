...
type
  TGoalRanking = class(TJanuaRecord, IJanuaRecord, IGoalRanking)
  private
    FieldIndex: Integer;
    FRownum: IJanuaField;
...
procedure TGoalRanking.CalcFields(Sender: TObject);
begin
  if PlayerId.AsInteger > 0 then
  begin
    FFullTeamName.AsString := '<img src="/?page=image&file=' + FTeamId.AsString + '.jpg"  alt="' + FTeamName.AsString +
      '" style="width:32px; height:32px"> ' + FTeamName.AsString;
    self.FFullName.AsString := self.FamilyName.AsString + ' ' + self.FFirstName.AsString;
  end;
end;
...
begin
  ...
  FieldIndex := AddField(FPlayerId);
  FGoals := TJanuaOrmFactory.CreateLargeIntField('goals', 'goals');
  FieldIndex := AddField(FGoals);
  FGoal := TJanuaOrmFactory.CreateLargeIntField('goal', 'goal');
  FieldIndex := AddField(FGoal);
  self.FFullName := TJanuaOrmFactory.CreateStringField('fullname', 'fullname');
  FieldIndex := AddField(FFullName);
  FFullName.Calculated := True;
  self.FullTeamName := TJanuaOrmFactory.CreateStringField('fullteamname', 'fullteamname');
  FieldIndex := AddField(FullTeamName);
  self.FullTeamName.Calculated := True;
  self.FFirstName.IsMonitored := True;
  self.FFamilyName.IsMonitored := True;
  // imposto il calcolo
...
function TGoalRanking.GetFullName: IJanuaField;
begin
  Result := self.FFullName
end;

function TGoalRanking.GetFullTeamName: IJanuaField;
begin
  Result := self.FFullTeamName
end;
...
procedure TGoalRanking.SetFullTeamName(const Value: IJanuaField);
begin
  self.FFullTeamName := Value;
end;
...
function TGoalRankingTable.GetFullName: IJanuaField;
begin
  Result := self.GoalRanking.FullName
end;
...
procedure TGoalRankingTable.SetFullName(const Value: IJanuaField);
begin
...
class function TGoalRankingFactory.CreateRecordset(const aName: string;
  const aLocalStorage, aRemoteStorage: IJanuaRecordSetStorage): IGoalRankingTable;
begin
