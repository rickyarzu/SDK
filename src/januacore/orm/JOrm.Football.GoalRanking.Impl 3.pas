...
begin
  self.FFullName.AsString := self.FamilyName.AsString + ' ' + self.FFirstName;
end;
...
begin
  ...
  self.FFullName := TJanuaOrmFactory.CreateLargeIntField('fullname', 'fullname');
  self.FFullName.Calculated := True;
  self.FFirstName.IsMonitored := True;
  self.FFamilyName.IsMonitored := True;
  self.FOnChangeActive := True;
  FFullNameIndex := AddField(FFullName);
end;
