...
uses Janua.Orm.Intf, Janua.Orm.Impl, JOrm.Football.Matches.Intf, Janua.Html.Intf;
...
type
  ...
  private
    ...
    FScore: IJanuaField;
    FButtons: IJanuaField;
    FHtmlNotes: IJanuaField;
    FMatchLevel: IJanuaField;
  private
  ...
  TMatchFactory = class
    class function CreateRecord(const aKey: string): IMatch; overload;
...
uses Spring, Janua.Html.Impl, Janua.Orm.Types;
...
var
  LUrl: string;
begin
  if self.FMatchId.AsLargeInt > 0 then
  begin
    ...
    FVisitorsTeamLogo.AsString := '<img src="/?page=image&file=' + FVisitorsTeamId.AsString + '.jpg"  alt="' +
      FVisitorsTeamName.AsString + '" style="width:32px; height:32px"> ' + FVisitorsTeamName.AsString;
    FButtons.AsString := '<a style="color:#fff;" class="m-badge  m-badge--info m-badge--wide">Tab</a>' + sLineBreak +
      '<a style="color:#fff;" class="m-badge  m-badge--danger m-badge--wide">Video</a>';
    self.FScore.AsString := self.FGoalHome.AsString + ' - ' + self.FGoalVisitor.AsString;
...
begin
  inherited;
  self.FPrefix := 'fcld';
  SetEntity(TJanuaEntity.FtCalendar);
  ...
  FItemIndex := AddField(FVisitorsTeamLogo);
  self.OnCalcFields := CalcFields;
  self.FHomeTeamId.IsMonitored := True;
  self.FVisitorsTeamId.IsMonitored := True;
  FScore := TJanuaOrmFactory.CreateStringField('score', 'score');
  FScore.Calculated := True;
  FItemIndex := AddField(FScore);
  FButtons := TJanuaOrmFactory.CreateStringField('buttons', 'buttons');
  FButtons.Calculated := True;
  FItemIndex := AddField(FButtons);
  self.FHtmlNotes := TJanuaOrmFactory.CreateStringField('htmlnotes', 'htmlnotes');
  FHtmlNotes.Calculated := True;
  FItemIndex := AddField(FHtmlNotes);
  FMatchLevel := TJanuaOrmFactory.CreateIntegerField('match_level', 'match_level');
  FItemIndex := AddField(FMatchLevel);
  self.FGoalHome.IsMonitored := True;
  self.FGoalVisitor.IsMonitored := True;
...
function TMatch.GetMatchLevel: IJanuaField;
begin
  Result := self.FMatchLevel
end;
...
procedure TMatch.SetMatchLevel(const Value: IJanuaField);
begin
  self.FMatchLevel := Value
end;
...
function TMatch.GetScore: IJanuaField;
begin
  Result := self.FScore
end;
...
procedure TMatch.SetScore(const Value: IJanuaField);
begin
  FScore := Value;
end;
...
function TMatch.GetHtmlNote: IJanuaField;
begin
  Result := FHtmlNotes;
end;
...
procedure TMatch.SetHtmlNote(const Value: IJanuaField);
begin
  FHtmlNotes := Value;
end;
...
function TMatch.MatchStatus(IsSmallScreen: boolean = False): string;
begin
  case self.MatchLevel.AsInteger of
    1:
      if IsSmallScreen then
        Result := '<h4 class=" text-center m--font-focus">' + 'LIVE' + '</h4>'
      else
        Result := '<h1 class=" text-center m--font-focus ">' + 'LIVE' + '</h1>';
    2:
      if IsSmallScreen then
        Result := '<h4 class=" text-center m--font-success ">' + 'FINALE' + '</h4>'
      else
        Result := '<h1 class=" text-center m--font-success ">' + 'FINALE' + '</h1>';
    ...
    3:
      if IsSmallScreen then
        Result := '<p class=" text-center  m--font-info "><strong>' + self.MatchDate.AsString + '</strong</p>'
      else
        Result := '<h4 class=" text-center  m--font-info ">' + self.MatchDate.AsString + '</h4>';
...
function TMatch.MatchStatusHtml(IsSmallScreen: boolean): IJanuaHtmlObject;
var
  aHelper: IHtmlHelper;
begin
  aHelper := THtmlHelper.Create;
  if self.Postponed.AsBoolean then
  begin
    if IsSmallScreen then
    begin
      Result := TJanuaHtmlObject.Create(aHelper, 'h4', ['text-center', ' m--font-focus']);
      Result.Text := 'RINVIATA';
    end
    else
    begin
      Result := TJanuaHtmlObject.Create(aHelper, 'h1', ['text-center', ' m--font-focus']);
      Result.Text := 'RINVIATA';
    end;
  end
  else
  begin
    case self.MatchLevel.AsInteger of
      ...
      2:
        if IsSmallScreen then
        begin
          Result := TJanuaHtmlObject.Create(aHelper, 'h4', ['text-center', ' m--font-success']);
          Result.Text := 'FINALE';
        end
        else
        begin
          Result := TJanuaHtmlObject.Create(aHelper, 'h1', ['text-center', ' m--font-success']);
          Result.Text := 'FINALE';
        end;
      3:
        if IsSmallScreen then
        begin
          Result := TJanuaHtmlObject.Create(aHelper, 'p', ['text-center', ' m--font-info']);
          Result.Text := MatchDate.AsString;
        end
        else
        begin
          Result := TJanuaHtmlObject.Create(aHelper, 'h4', ['text-center', ' m--font-info']);
          Result.Text := MatchDate.Format('dd/mm/yyyy hh:nn');
        end
    else
      Result := TJanuaHtmlObject.Create(aHelper, 'p', '', False);
    end;
  end;
...
function TMatch.GetButtons: IJanuaField;
begin
  Result := self.FButtons
end;
...
procedure TMatch.SetButtons(const Value: IJanuaField);
begin
  FButtons := Value
end;
...
function TMatches.GetScore: IJanuaField;
begin
  Result := self.Match.Score
end;
...
procedure TMatches.SetScore(const Value: IJanuaField);
begin
  self.Match.Score := Value
end;
...
class function TMatchFactory.CreateRecord(const aKey: string): IMatch;
begin
  Result := TMatch.Create;
end;

class function TMatchFactory.CreateRecordset(const aName: string;
  const aLocalStorage, aRemoteStorage: IJanuaRecordSetStorage): IMatches;
begin
