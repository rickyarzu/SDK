unit Janua.UnitTest.ViewModel.System;
...
begin
{$IFDEF TESTPOSTGRES}
  lUser := 'newuser' + FormatDateTime('YYMMDDHHNNSSZZZ', Now()) + '@gmail.com';
{$ELSE}
  lUser := 'newuser@gmail.com';
{$ENDIF TESTPOSTGRES}
  ...
  aUserProfile.Clear;
  aUserProfile.User.Username.AsString := lUser;
  aUserProfile.User.Password.AsString := 'newpassword';
  bTest := LDBModel.CheckUser(aUserProfile);
{$IFDEF TESTPOSTGRES}
  Assert.AreEqual(True, bTest, 'Error user registered ' + lUser);
{$ENDIF TESTPOSTGRES}
  try
    bTest := LDBModel.AddUser(aUserProfile);
{$IFDEF TESTPOSTGRES}
    Assert.AreEqual(True, bTest, 'Error adding user ' + lUser);
{$ENDIF TESTPOSTGRES}
    bTest := LDBModel.Login(aUserProfile.User);
{$IFDEF TESTPOSTGRES}
    Assert.AreEqual(True, bTest, 'Login Failed ' + lUser);
...
{$IFDEF TESTPOSTGRES}
    ...
    Assert.AreEqual(True, LDBModel.Login(aUserProfile.User), '2nd Login Failed ' + lUser);
    Assert.AreEqual(lTest, LDBModel.AnagraphProfile.AnagraphId.AsInteger, '2nd AnagraphProfile.AnagraphId');
{$ENDIF TESTPOSTGRES}
  finally
{$IFDEF TESTPOSTGRES} LDBModel.DelUser(lUser); {$ENDIF TESTPOSTGRES}
  end;
...
{$IFDEF TESTPOSTGRES}
    Assert.AreEqual(True, LDBViewModel.SilentLogin(aUserProfile.User), 'Login Failed ' + lUser);
...
{$IFDEF TESTPOSTGRES}
    ...
    Assert.AreEqual(lTest, LDBViewModel.AnagraphProfile.AnagraphId.AsInteger,
      'LDBModel.AnagraphProfile.AnagraphId');
    Assert.AreEqual(True, LDBViewModel.SilentLogin(aUserProfile.User), '2nd Login Failed ' + lUser);
    Assert.AreEqual(lTest, LDBViewModel.AnagraphProfile.AnagraphId.AsInteger,
      '2nd AnagraphProfile.AnagraphId');
{$ENDIF TESTPOSTGRES}
  finally
{$IFDEF TESTPOSTGRES} LDBViewModel.DelUser(lUser); {$ENDIF TESTPOSTGRES}
  end;
end;
...
begin
  ...
  LDBViewModel.Activate;
  if expected then
    aUsername := FormatDateTime('YYMMDDHHNNSSZZZ', Now()) + aUsername;
...
{$ENDIF TESTPOSTGRES}
  ...
  begin
    ...
    Assert.AreNotEqual(0, lID, 'user not registered is 0 instead of N');
...
begin
  ...
  Assert.IsNotNull(lDataModule.jspAddAnagraphProfile, 'jspAddAnagraphProfile');
