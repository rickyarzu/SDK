...
{$ENDIF}
  ...
  // Janua ORM Framework
  JOrm.System.UserProfile.Custom.Intf, JOrm.System.Intf, JOrm.System.Impl,
...
begin
  ...
  // Creo una variabile Interfaccia IUser locale denominata LUser
  LUser := TUsersFactory.CreateRecord('user');
...
begin
  ...
  try
    ...
    LTestParam := LParams.ParamByName('username');
    Assert.IsNotNull(LTestParam, 'LTestParam username');
    Assert.AreEqual(aUsername, LTestParam.AsString, 'Param username: ' + LTestParam.AsString + '/' +
      aUsername);
    LTestParam := nil;
    LTestParam := LParams.ParamByName('password');
    ...
    on e: exception do
      TJanuaLogger.LogError('TestSystem.Login', Format('(%s, %s)', [aUsername, aPassword]), nil, e);
  end;
