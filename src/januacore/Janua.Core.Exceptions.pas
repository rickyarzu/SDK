...
begin
  ...
  FContenuto := 'ERRORE DI ACCESSO ALLA MEMORIA : ' + sl +
    ...
    '1. La memoria del sistema non è sufficiente a gestire le applicazioni attualmente attive.' + sl +
    '2. Il programma ha cercato di accedere ad una componente che non è stata trovata.' + sl +
    ...
    sl + '3. Riavviare il computer' + sl + '4. Entrare nel programma e verificare le funzionalità principali'
...
begin
  ...
  begin
    if (pos('PDMST_UK_AU_TRG', stringa) > 0) then
      ...
      FRiga := ('Attenzione, è stata immessa una targa omonima nella polizza. (codice: NUOVA_TARGA J001)' + sl
        + 'premere Esc per annullare l''inserimento')
    else if pos('ORDVEI_TARGA_UK', stringa) > 0 then
      FRiga := ('Attenzione, è stata immessa una targa omonima nella prenotazione.' + sl +
      ...
      FRiga := ('Attenzione, è stata immessa una partita iva duplicata (ORA-00001 AN_PI_CF).' + sl +
        'premere Esc per annullare l''inserimento')
  ...
  begin
    FRiga := ('Attenzione, è stato inserito un utente o una password errata.' + sl +
      'è possibile eseguire non più di 3 tentativi.....:');
    Result := True;
  end
  else if errore = '02291' then
  begin
    FRiga := ('Attenzione, è stato inserito un valore non presente in una tabella di decodifica.' + sl +
  ...
  begin
    FRiga := ('Attenzione state cercando modficare una polizza già contabilizzata.' + sl +
      'Questo errore si verifica quando una polizza è già stata passata in contabilità.' + sl +
      'Se è necessario effettuare una modifica potete richiederla in sede ...' + sl +
  ...
  begin
    FRiga := ('Attenzione state cercando di realizzare un ritorno non valido.' + sl +
      'Questo errore si verifica quando esiste già una prenotazione di ritorno valida.' + sl +
  ...
  begin
    FRiga := ('Attenzione state cercando di realizzare un ritorno non valido. (ORA-20111 SH_PKG)' + sl +
      'Questo errore si verifica quando esiste già una polizza di ritorno valida.' + sl +
  ...
  begin
    FRiga := ('Attenzione state cercando di ELIMINARE o annullare un documento contabilizzato.' + sl +
      'Questo errore si verifica quando è già stato inviato in contabilità o prima nota.' + sl +
  ...
  begin
    FRiga := ('Attenzione state cercando di modificare o cancellare una scrittura in prima nota.' + sl +
      'La scrittura è in un mese già chiuso e contabilizzato.' + sl +
  ...
  begin
    FRiga := ('Attenzione questo client non è collegato ad Oracle. (ORA-01012)' + sl +
  ...
  begin
    FRiga := ('Attenzione questo client non è collegato ad Oracle. (ORA-01033)' + sl +
  ...
  begin
    FRiga := ('Attenzione questo client non è collegato ad Oracle. (ORA-03113)' + sl +
      'è caduta la linea di collegamento.');
    Result := True;
  end
  else if errore = '03114' then
  begin
    FRiga := ('Attenzione questo client non è collegato ad Oracle. (ORA-03114)' + sl +
      'è caduta la linea di collegamento.');
    Result := True;
  end
  else if errore = '12571' then
  begin
    FRiga := ('Attenzione questo client non è collegato ad Oracle. (ORA-12571)' + sl +
...
begin
  FContenuto := 'Istruzioni : ' + sl +
    ...
    + sl + '1. Verificare i campi chiave (di solito campi "Codice", "Targa",  "Matricola"), per rendersi conto se si è cercato di inserire un codice già presente'
    + sl + '2. La verifica può essere fatta variando anche solo un carattere del campo chiave e provando a confermare la modifica con il segno di spunta presente nella pulsantiera.'
    ...
    + sl + '4. Se anche con questo sistema non si risolve il problema contattare il servizio assistenza che vi guiderà alla soluzione.';
...
begin
  ...
  begin
    ...
    begin
      FRiga := (
        ...
        + sl + 'Questo simbolo verrà aggiunto dal programma ma non può essere inserito dall''utente. Cancellatelo e riprovate');
      risultato := True
    end
    ...
    begin
      FRiga := (
        ...
        + sl + 'Ma voi avete inserito un punto: ".", non sono ammessi punti nei campi di valore numerico né come separatori di migliaia né come separatori decimali.');
      risultato := True
    end
    else
    begin
  ...
  begin
    FRiga := (
      ...
      + sl + 'Altrimenti i vostri inserimenti successivi non potranno essere più passati in archivio. Errore: DisposeBuf failed');
    risultato := True
  end;
  ...
  begin
    FRiga := (
      ...
      + sl + 'Il sistema attenderà 10 secondi per continuare, se questo errore dovesse continuare controllate la cartela stampanti'
  ...
  if not risultato then
    FRiga := ('Si è verificato un errore:' + sl + FRiga + sl +
