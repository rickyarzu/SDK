...
{ This procedure loads the tDecimalRoundingCtrl descriptions and ordinals
  into the string list for such use as using a TRzCombobox_go to make rounding
  type selection. }
...
Const
DecimalRoundingCtrlStrs:
{$IFDEF MSWINDOWS}
  array [tDecimalRoundingCtrl] of record Abbr: string[9];
Dscr:
string[59];
{$ELSE}
  array [tDecimalRoundingCtrl] of record Abbr: string;
Dscr:
string;
{$ENDIF MSWINDOWS}
end
= ((Abbr: 'None'; Dscr: 'No rounding.'), (Abbr: 'HalfEven';
  Dscr: 'Round to nearest or to even whole number' + ' (a.k.a Bankers) '), (Abbr: 'HalfPos';
  Dscr: 'Round to nearest or toward positive'), (Abbr: 'HalfNeg'; Dscr: 'Round to nearest or toward negative'),
  (Abbr: 'HalfDown'; Dscr: 'Round to nearest or toward zero'), (Abbr: 'HalfUp';
  Dscr: 'Round to nearest or away from zero'), (Abbr: 'RndNeg'; Dscr: 'Round toward negative. (a.k.a. Floor) '),
  (Abbr: 'RndPos'; Dscr: 'Round toward positive. (a.k.a. Ceil ) '), (Abbr: 'RndDown';
  Dscr: 'Round toward zero. (a.k.a. Trunc) '), (Abbr: 'RndUp'; Dscr: 'Round away from zero.'));
...
Function DecimalRound(Value: extended; NDFD: integer; MaxRelErr: double; Ctrl: tDecimalRoundingCtrl = drHalfEven)
  : extended;
...
begin
  ...
  Assert(MaxRelErr > 0, 'MaxRelErr param in call to DecimalRound() must be greater than zero.');

  { Compute 10^NDFD and scale the Value and MaxError: }
  m := 1;
  For k := 1 to abs(NDFD) do
    m := m * 10;
  If NDFD >= 0 then
  begin
    ScaledVal := Value * m;
...
{$IFNDEF FPC}
    ...
    begin
      Strings.AddObject(String(DecimalRoundingCtrlStrs[dr].Abbr), pointer(dr));
    end;
{$ENDIF FPC}
  end;
