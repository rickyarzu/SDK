...
uses
  System.SysUtils, System.Classes, Janua.Data.Customsession,  Data.DB, DBAccess,
   PgAccess, Datasnap.DBClient,  MemDS;

type
  TJanuaPgSession = class(TJanuaCustomSession)
  private
    FspLogin: TPgStoredProc;
    FPgConnection: TPgConnection;
    procedure SetPgConnection(const Value: TPgConnection);
    procedure SetspLogin(const Value: TPgStoredProc);
  protected
  ...
  published
    property PgConnection: TPgConnection read FPgConnection write SetPgConnection;
    property spLogin: TPgStoredProc read FspLogin write SetspLogin;
...
begin
  Result := False;
 { TODO -oRiccardo -cSystem : Implementare funzione Login With Session }
end;
...
procedure TJanuaPgSession.SetPgConnection(const Value: TPgConnection);
begin
  FPgConnection := Value;
end;

procedure TJanuaPgSession.SetspLogin(const Value: TPgStoredProc);
begin
  FspLogin := Value;
