...
begin
  if not(self.isInitializing or (csDesigning in self.ComponentState)) then
    try
      Assert(Assigned(self.FFootballSettings),
        'TJanuaServerPgFootball.CreateDataModule IFootballSettings not set');
      Assert(Assigned(self.FJanuaSystem), 'TJanuaServerPgFootball.CreateDataModule FJanuaSystem not set');
      self.FActive := True;

      if self.FActive and not Assigned(self.DM) then
      begin
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      self.WriteError('TJanuaServerPgFootball.InternalActivate', E);
    end;
...
begin
  inherited;
  Assert(Assigned(self.FFootballSettings),
    'TJanuaServerPgFootball.InternalOpenCalendar IFootballSettings not set');
  DM.OpenCalendar;
  self.AssignDatasets;
end;
...
begin
  ...
  Assert(Assigned(FFootballSettings),
    'TJanuaServerPgFootball.InternalOpenNextMatches FFootballSettings not Assigned');
  SetSeasonLeagueDay;
...
begin
  ...
  Assert(Assigned(FFootballSettings),
    'TJanuaServerPgFootball.InternalOpenPlayerCareer FFootballSettings not Assigned');
...
begin
  ...
  Assert(Assigned(FFootballSettings),
    'TJanuaServerPgFootball.InternalOpenRanking FFootballSettings not Assigned');
  SetSeasonLeagueDay;
  DM.OpenRanking;
  self.AssignDatasets;
  {
...
begin
  ...
  Assert(Assigned(FFootballSettings),
    'TJanuaServerPgFootball.InternalOpenSeasonsList FFootballSettings not Assigned');
  self.DM.OpenSeasons;
...
procedure TJanuaPgWebSessionFootball.SetJanuaPgFootballWebController
  (const Value: TJanuaPgFootballWebController);
begin
