...
uses
  ...
  Janua.Core.Classes,
  Janua.Core.System, Janua.Core.Mail, Janua.Postgres.System, Janua.Postgres.JPublic;
...
function AddDoctor(p_firstname, p_lastname, p_organisation_number: string; p_organisation_date: TDate;
  p_organisation_id, p_district_id, p_council_id: integer; prcCreateNewDoctor: TPgStoredProc): boolean;
begin
...
begin
  ...
  if self.CheckUp and Result then
  begin
...
begin
  ...
  begin
    ...
    self.IDM := DM;
    self.WriteLog('TJanuaServerHealthBooking.JanuaCoreLogger');
...
function TJanuaHealthPgWebController.Activate: boolean;
begin
  Result := self.InternalActivate;
end;
