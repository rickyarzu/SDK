...
uses
  ...
  Janua.Core.Functions, Janua.Core.Classes, Janua.Core.System, Janua.Core.Cloud,
  ...
  udmPgAnagraph, Janua.Postgres.Cloud, udmJanuaPostgresModel, Janua.Postgres.Functions, Janua.Core.Exceptions, DAScript,
  PgScript;
...
type
  TdmFootballPostgres = class(TdmJanuaPostgresModel)
    ...
    lkpMatchDaysFrom: TPgQuery;
    lkpMatchDaysTo: TPgQuery;
    lkpMatchDaysFrommatch_day_number: TSmallintField;
    lkpMatchDaysTomatch_day_number: TSmallintField;
    prcInheritPlayer: TPgStoredProc;
    qryLkpAllPlayers: TPgQuery;
    ...
    PgScript1: TPgScript;
    prcPlayerTeamOrigin: TPgStoredProc;
...
begin
  ...
  begin
    ...
    begin
      self.LastErrorMessage := 'Attenzione non inserita formazione del: ' + qryMatchTeamsteam_name.AsWideString;
        ...
        begin
          ...
          begin
            ...
            GoalKeepersHome[i] := TJanuaFootballPlayer.Create(qryMatchesseason_id.AsInteger,
              qryMatchesleague_id.AsInteger, qryMatcheshome_team_id.AsInteger, qryMatchTeamPlayersplayer_id.AsInteger,
              qryMatchTeamPlayersrole_id.AsInteger, self.qryMatchTeamPlayerssince.AsInteger,
              self.qryMatchTeamPlayersuntil.AsInteger);
            ...
            self.WriteLog('Inserito portiere Casa: ' + GoalKeepersHome[i].FullName + ' dal: ' + GoalKeepersHome[i]
              .SelectedCareer.playsince.ToString + ' al: ' + GoalKeepersHome[i].SelectedCareer.playuntil.ToString);
          end
          else
          begin
            ...
            self.WriteLog('Inserito portiere Ospiti: ' + GoalKeepersVisitors[i].FullName + 'dal: ' + GoalKeepersVisitors
              [i].SelectedCareer.playsince.ToString + ' al: ' + GoalKeepersVisitors[i]
        ...
        begin
          ...
          begin
            if (GoalKeepersVisitors[i].SelectedCareer.playsince <= self.qryMatchEventsminute.AsInteger) and
              ...
              (self.qryMatchEventsminute.AsInteger > 90) and (GoalKeepersVisitors[i].SelectedCareer.playuntil = 90))
            then
            begin
        ...
        begin
          ...
          begin
            if (GoalKeepersHome[i].SelectedCareer.playsince <= self.qryMatchEventsminute.AsInteger) and
              ...
              (self.qryMatchEventsminute.AsInteger > 90) and (GoalKeepersHome[i].SelectedCareer.playuntil = 90)) then
            begin
              GoalKeepersHome[i].lostgoal := GoalKeepersHome[i].lostgoal + 1;
...
begin
  ...
  begin
    ...
    FJanuaServerCloud.JanuaLogger := self.JanuaCoreLogger;
...
begin
  try
    ...
    // Janua.Postgres.Functions.OpenPgThreadedDataset(qrySeasons, dsSeasons);
    // Janua.Postgres.Functions.OpenPgThreadedDataset(qryChampionships);
    // Janua.Postgres.Functions.OpenPgThreadedDataset(lkpTeams);
    // self.ServerFunctions.OpenDataset(qrySeasons);
    Janua.Postgres.Functions.OpenPgThreadedDataset(lkpMatchDaysFrom);
    Janua.Postgres.Functions.OpenPgThreadedDataset(lkpMatchDaysTo);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qrySeasons, dsSeasons);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryChampionships, self.dsChampionships);
    ...
    Janua.Postgres.Functions.OpenPgThreadedDataset(lkpTeams);
    Janua.Postgres.Functions.OpenPgThreadedDataset(cdsLkpClubs);
    Janua.Postgres.Functions.OpenPgThreadedDataset(cdsLkpClubs);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryClubs, self.dsClubs);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryTeams, self.dsTeams);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryTeamPlayers, self.dsTeamPlayers);

    {
      self.ServerFunctions.OpenDataset(lkpTeams);
      self.ServerFunctions.OpenDataset(cdsLkpClubs);
      self.ServerFunctions.OpenDataset(cdsLkpClubs);
      self.ServerFunctions.OpenDataset(lkpRoles);
      self.ServerFunctions.OpenDataset(qryTeams);
      self.ServerFunctions.OpenDataset(qryClubs);
      self.ServerFunctions.OpenDataset(qryTeamPlayers);
    }

    Janua.Postgres.Functions.OpenPgThreadedDataset(qryTeamsChampionships);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryMatchDays);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryMatches);
    Janua.Postgres.Functions.OpenPgThreadedDataset(cdsEventTypes);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryMatchTeams);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryMatchAllPlayers);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryMatchEvents);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryChampRanking);
    Janua.Postgres.Functions.OpenPgThreadedDataset(lkpTeams);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryTeamRankings);
    Janua.Postgres.Functions.OpenPgThreadedDataset(lkpTeamsChampionship);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryPlayerHistory);
    Janua.Postgres.Functions.OpenPgThreadedDataset(qryPlayerAlbum);
    /// self.qryAllPlayers.Open;
    /// {non aprire per non sovracaricare il sistema}
    Janua.Postgres.Functions.OpenPgThreadedDataset(self.qryLkpAllPlayers, dsLkpAllPlayers);
  except
    on e: exception do
    begin
...
begin
  try
    ...
    begin
      ...
      self.qryTeamChampPlayers.ParamByName('team_id').AsInteger := self.qryTeamsChampionshipsteam_id.AsInteger;
