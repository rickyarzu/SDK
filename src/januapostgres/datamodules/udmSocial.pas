...
uses
  ...
  Janua.Core.System, Janua.Postgres.Functions, Janua.Core.Social,
  Janua.Core.Classes,
  udmJanuaPostgresModel, Datasnap.Provider, Datasnap.DBClient;

type
  TdmSocial = class(TdmJanuaPostgresModel)
    spInsertMessage: TPgStoredProc;
    qryMessages: TPgQuery;
    spDeleteMessage: TPgStoredProc;
    qryMessageCount: TPgQuery;
    qryMessageCountmessages: TLargeintField;
    qryMessagesNotRead: TPgQuery;
    ...
    qryMessagesNotReadmsg_datetime: TSQLTimeStampField;
    ...
    qryMessagesmsg_datetime: TSQLTimeStampField;
...
begin
  if self.FUserProfile.User.ID > 0 then
  begin
    self.qryMessageCount.Close;
    self.qryMessageCount.ParamByName('p_to_id').AsInteger := FUserProfile.User.ID;
