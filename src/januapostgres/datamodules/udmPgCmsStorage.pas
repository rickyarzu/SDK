...
uses
  System.SysUtils, System.Classes, Data.DB, DBAccess, PgAccess, Janua.Orm.Intf, MemData, MemDS, udmPgStorage,
  Janua.Postgres.Functions, JOrm.Cms.Articles.Intf, Datasnap.Provider, Datasnap.DBClient;

type
  TdmPgCmsStorage = class(TdmPgStorage)
    main_arguments: TPgQuery;
    dsMainArguments: TPgDataSource;
    arguments: TPgQuery;
    ...
    scMainArgumentsInsert: TPgSQL;
    scMainArgumentsDelete: TPgSQL;
    scMainArgumentsUpdate: TPgSQL;
    qryArticlesList: TPgQuery;
    qryArgumentsArticles: TPgQuery;
    ...
    scArticlesInsert: TPgSQL;
    scArticlesUpdate: TPgSQL;
    scArticlesDelete: TPgSQL;
    ...
    dsArgumentsArticles: TPgDataSource;
    ...
    Article: TPgQuery;
    scArgumentsInsert: TPgSQL;
    scArgumentsDelete: TPgSQL;
    scArgumentsUpdate: TPgSQL;
    ...
    qryArgumentsChampionships: TPgQuery;
    qryLeagues: TPgQuery;
    ...
    Articledatetime: TSQLTimeStampField;
    ...
    dsArticlesList: TPgDataSource;
    qryArticlesAttachments: TPgQuery;
    ...
    dsArticle: TPgDataSource;
    ArticleAttachments: TPgQuery;
    ...
    qryArgumentArticlesAttachments: TPgQuery;
    qryWarnings: TPgQuery;
    ...
    qryWarningsdatetime: TSQLTimeStampField;
    ...
    qryMainArgumentsMails: TPgQuery;
  ...
  public
    ...
    procedure OpenArticles(aMainArgumentID, aArgumentID, aLimit, aPage: Word; aHasLink: boolean = False); overload;
...
uses
  Janua.Core.Functions, Janua.Core.Types, Janua.Orm.UniDac, JOrm.Cms.Articles.Impl, System.StrUtils,
  System.Math;
...
begin
  self.arguments.ParamByName('db_schema_id').AsInteger := JanuaApplicationGlobalProfile.DBSchemaID
end;
...
begin
  ...
  if argumentsdb_schema_id.AsInteger = 0 then
    argumentsdb_schema_id.AsInteger := JanuaApplicationGlobalProfile.DBSchemaID;
...
begin
  inherited;
  PgFunctions.OpenDataset(ArticleAttachments)
end;
...
begin
  ...
  if dmPgCmsStorage.cdsArgumentsArticleslanguage_id.AsInteger = 0 then
    dmPgCmsStorage.cdsArgumentsArticleslanguage_id.AsInteger := JanuaApplicationGlobalProfile.LanguageID;
  ...
  if dmPgCmsStorage.cdsArgumentsArticlesauthor_id.AsInteger = 0 then
    dmPgCmsStorage.cdsArgumentsArticlesauthor_id.AsInteger := JanuaApplicationGlobalProfile.DBUserID;
  if dmPgCmsStorage.cdsArgumentsArticlesdb_schema_id.AsInteger = 0 then
    dmPgCmsStorage.cdsArgumentsArticlesdb_schema_id.AsInteger := JanuaApplicationGlobalProfile.DBSchemaID;

  if dmPgCmsStorage.cdsArgumentsArticlesarticle_id.AsLargeInt = 0 then
  begin
    ServerFunctions.OpenDataset(qryArticlesSeq);
    dmPgCmsStorage.cdsArgumentsArticlesarticle_id.AsLargeInt := dmPgCmsStorage.qryArticlesSeqnextval.AsLargeInt;
...
begin
  ...
  if cdsArgumentsdb_schema_id.AsInteger = 0 then
    cdsArgumentsdb_schema_id.AsInteger := JanuaApplicationGlobalProfile.DBSchemaID;
...
begin
  ...
  if cdsMainArgumentsdb_schema_id.AsInteger = 0 then
    cdsMainArgumentsdb_schema_id.AsInteger := JanuaApplicationGlobalProfile.DBSchemaID;
...
procedure TdmPgCmsStorage.DataModuleCreate(Sender: TObject);
begin
  ServerFunctions := Janua.Postgres.Functions.TJanuaPostgresServerFunctions.Create(self);
  // (aDataset: TCustomPgDataSet;  aInsertObject, aUpdateObject, aDeleteObject: TPgSql; aRefreshObject: TPgQuery
end;
...
function TdmPgCmsStorage.GetArticlesList(aMainArgumentID, aArgumentID, aLimit, aPage: Word; aIsVideo: boolean = False)
  : IArticles;
VAR
  tmp: integer;
begin
  ...
  execProc(
    ...
    begin
      ...
      if main_argumentsdb_schema_id.AsInteger = 0 then
        main_argumentsdb_schema_id.AsInteger := JanuaApplicationGlobalProfile.DBSchemaID;
...
begin
  ...
  {
    if (DataSet is TPgQuery) then
    with (DataSet as TPgQuery) do
    begin
...
begin
  ...
  PgFunctions.OpenDataset(qryArticlesList);

end;

procedure TdmPgCmsStorage.OpenArticles(aMainArgumentID, aArgumentID, aLimit, aPage: Word; aHasLink: boolean = False);
var
  limit, offset: Word;
begin
  {
...
begin
  execProc(
    ...
    begin
      ...
      PgFunctions.OpenDataset(main_arguments);
      PgFunctions.OpenDataset(arguments);
    end, 'OpenMainArguments', self);

end;

procedure TdmPgCmsStorage.PgErgoConnectionBeforeConnect(Sender: TObject);
begin
...
begin
  if (Sender is TPgSQL) then
    with (Sender as TPgSQL) do
    begin
      ParamByName('db_schema_id').AsInteger := JanuaApplicationGlobalProfile.DBSchemaID
    end;
end;
...
function TdmPgCmsStorage.GetArticlesListPos(aMainArgumentID, aPosition, aLimit, aPage: Word; aHasLink: boolean = False)
  : IArticles;
begin
  Result := TArticles.Create;
