...
uses Janua.Orm.Intf, Janua.Server.Intf, Janua.Core.DB.Intf, Janua.Core.Types,
  // Orm
  JOrm.System.Intf,
  // Parent
...
type
  ...
  strict private
    FFDM: TdmPgSystemStorage;
    function FDM: TdmPgSystemStorage;
  strict protected
...
uses System.SysUtils, Janua.Orm.UniDac, Data.DB, Janua.Core.Functions, Spring, System.StrUtils,
  Janua.Application.Framework, Janua.Orm.Types;
  ...
  try
    ...
    begin
      if (aUser.User.SocialID.AsString = '') then
        ...
        Result := InternalAddUserSocial(aUser);
    ...
    // è veramente un utente amministratoe ed ha i diritti di farlo
    begin
      ...
      FDM.qryGetUserByKey.ParamByName('verify_token').AsString := aUser.User.VerifyToken.AsString;
      ...
      begin
        ...
        begin
          ...
          FDM.spSetUserDefaultSchema.ParamByName('p_session_key').AsWideString := self.GetSessionKey;
          FDM.spSetUserDefaultSchema.ParamByName('p_user_id').AsInteger := aUser.User.DbUserId.AsInteger;
          FDM.spSetUserDefaultSchema.Execute;
        end;
        FDM.ServerFunctions.OpenDataset(FDM.qryGetUserByKey);
...
begin
  Guard.CheckTrue((aUser.Db_user_id.AsInteger > 0) or (aUser.User.Email.AsString <> ''),
    'DelUser user not set');
  if aUser.Db_user_id.AsInteger > 0 then
    Result := FindUserByID(aUser.Db_user_id.AsInteger)
  else
    Result := SearchUserByEmail(aUser.User.Email.AsString);
  if Result then
    Result := DelUser;
end;
...
begin
  ...
  try
    ...
    try
      ...
      on e: exception do
        RaiseException('DoPaymentSearchUser', e, self, self.LogString);
    end;
  finally
...
function TJanuaPgSystemStorage.FDM: TdmPgSystemStorage;
begin
  ...
  try
    try
      ...
      *)
      ...
      on e: exception do
        RaiseException('GetSession', e, self, self.LogString);
    end;
  finally
...
begin
  ...
  try
    try
      Result := SetupLogin(aUser, LmailLogin);
      if Result then
      begin
        if LmailLogin then
        begin
...
begin
  try
    ...
    on e: exception do
      RaiseException('OpenUserProfile', e, self, LogString);
  end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('SaveProfile', e, self, LogString);
  end;
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('SearchUserByEmail', e, self, LogString);
  end;
end;
...
begin
  ...
  try
    try
      ...
      on e: exception do
        RaiseException('SearchUserByID', e, self, LogString);
    end;
  finally
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('SearchUserByName', e, self, self.LogString);
  end;
end;
...
procedure TJanuaPgSystemStorage.SetAfterCreateDataset;
begin
  inherited;

end;

procedure TJanuaPgSystemStorage.SetAfterDestroyDataset;
begin
  inherited;

end;

procedure TJanuaPgSystemStorage.SetInternalCreateDataset;
begin
  inherited;
  ...
  FInternalCreateDataset := procedure
    begin
      try
        ...
        on e: exception do
          RaiseException('InternalCreateDataset' { ProcName } , e, self { Class } );
      end;
    end;

end;

procedure TJanuaPgSystemStorage.SetInternalCreateDBDataSets;
begin
  inherited;
  ...
  FInternalCreateDBDataSets := procedure
    begin
      try
        WriteLocalLog('InternalCreateDBDataSets', 'SetjdsUsers(TJanuaPgDataset)');
        SetjdsUsers(TJanuaPgDataset.Create(FDM.qryUser, [TJanuaEntity.SysDbUsers]));
        WriteLocalLog('InternalCreateDBDataSets', 'SetjdsUserProfiles(TJanuaPgDataset)');
        SetjdsUserProfiles(TJanuaPgDataset.Create(FDM.qryUserProfile, [TJanuaEntity.SysDbUsersProfiles]));
        WriteLocalLog('InternalCreateDBDataSets', 'SetjdsUserSessions(TJanuaPgDataset)');
        SetjdsUserSessions(TJanuaPgDataset.Create(FDM.qryUserSession, [SysDbUserSessions, SysDbUsersProfiles,
          SysDbUsers]));
        WriteLocalLog('InternalCreateDBDataSets', 'SetjdsSearchUsers(TJanuaPgDataset)');
        SetjdsSearchUsers(TJanuaPgDataset.Create(FDM.qrySearchUsers, [TJanuaEntity.SysDbUsers]));
        WriteLocalLog('InternalCreateDBDataSets', 'SetjdsSearchUserProfiles(TJanuaPgDataset)');
        SetjdsSearchUserProfiles(TJanuaPgDataset.Create(FDM.qrySearchUserProfile, [SysDbUsersProfiles]));

      except
        on e: exception do
          RaiseException('InternalCreateDBDataSets' { ProcName } , e, self { Class } , LogString);
      end;
...
procedure TJanuaPgSystemStorage.SetInternalDestroyDataset;
begin
  inherited;
  ...
  FInternalDestroyDataset := procedure
    begin
      try
        ...
        on e: exception do
          RaiseException('InternalDestroyDataset' { ProcName } , e, self { Class } , LogString);
      end;
