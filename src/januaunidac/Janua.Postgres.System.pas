...
uses
  ...
  Janua.Core.DB.Intf, Janua.Core.Entities, Janua.Core.Forms, Janua.Core.Functions, Janua.Core.Classes;
...
begin
  ...
  if not Assigned(DM) then
    try
      ...
      {
        FIDM.ServerFunctions.Verbose := self.Verbose;
        FIDM.ServerFunctions.LogToFile := self.LogToFile;
        FIDM.ServerFunctions.ApplicationType := TJanuaApplication.ApplicationType;
      }
...
begin
  ...
  if not Result then
    try
      Result := inherited and LoadedProfile and self.Login;;
    except
      on e: exception do
...
begin
  ...
  begin
    ...
    { TODO : Verificare cosa succede su Activate e Login }
    self.Active := True;
...
begin
  ...
  self.DatasetErrors := self.FIDM.DatasetErrors;
  ...
  if Result then
    try
      ...
      try
        ...
        if not Result and (FAttempts < 3) and not TJanuaApplication.IsServer then
          self.Login;
      end;
    Except
      on e: exception do
      begin
        if Assigned(DM) then
          self.WriteLog('TJanuaPostgresSystem.InternalLogin', FIDM.log);
        Result := false;
...
begin
  ...
  if (self.SessionKey <> '') and Assigned(DM) then
    Result := DM.CheckSessionKey(aKey);
  if not Result then
    UndoSessionLogin;
end;
...
begin
  ...
  if not(self.UserProfile.User.Email > '') then
    LocalRaiseException('TJanuaPostgresSystem.NewProfile Username not Set');

  if not(self.UserProfile.User.Password > '') then
    LocalRaiseException('TJanuaPostgresSystem.NewProfile Password not Set');
  ...
  try
    ...
    try
      ...
      begin
        self.FHasErrors := True;
        self.LastErrorMessage := DM.LastErrorMessage;
        self.WriteLog('TJanuaPostgresSystem.NewProfile', FIDM.log);
      end;
...
begin
  if not(self.UserProfile.User.Email > '') and not(csDesigning in ComponentState) then
    LocalRaiseException('TJanuaPostgresSystem.SaveProfile Username not Set');
  ...
  if not(self.UserProfile.User.Password > '') and (UserProfile.User.id = 0) and
    ...
    LocalRaiseException('TJanuaPostgresSystem.SaveProfile Password not Set');
  ...
  try
    try
      ...
      if FcdsUsersList.RecordCount <> DM.qrySearchUsers.RecordCount then
        LocalRaiseException('Error SearchUser Record');
    finally
      self.DestroyDataModule;
    end;
  except
