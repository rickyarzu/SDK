...
uses
  ...
  udmJanuaPostgresModel, Datasnap.Provider, Datasnap.DBClient, Janua.Core.Exceptions, Uni, UniProvider,
  PostgreSQLUniProvider;

type
  TdmPgSwap = class(TdmJanuaPostgresModel)
    qrySwapProfile: TUniQuery;
    spEventsAddUser: TUniStoredProc;
    spEventsDelUser: TUniStoredProc;
    spItemInsert: TUniStoredProc;
    qryDiscoverItems: TUniQuery;
    qryDiscoverItemsid: TLargeintField;
    qryDiscoverItemsowner_id: TIntegerField;
    qryDiscoverItemsinsert_date: TDateTimeField;
    ...
    spItemLike: TUniStoredProc;
    spItemDiscovered: TUniStoredProc;
    qryEditItem: TUniQuery;
    qryUserItems: TUniQuery;
    qryUserItemsid: TLargeintField;
    qryUserItemsowner_id: TIntegerField;
    qryUserItemsinsert_date: TDateTimeField;
    ...
    qryWatchItemImagesinsert_date: TDateTimeField;
    spImageInsert: TUniStoredProc;
    qryImage: TUniQuery;
    ...
    qryImageinsert_date: TDateTimeField;
    spItemsDisLike: TUniStoredProc;
    spUserFollow: TUniStoredProc;
    qryUserFollowers: TUniQuery;
    qryUserLikes: TUniQuery;
    qryUserLikesid: TLargeintField;
    qryUserLikesowner_id: TIntegerField;
    qryUserLikesinsert_date: TDateTimeField;
    ...
    spSwapStart: TUniStoredProc;
    qryUserSwaps: TUniQuery;
    ...
    qryUserSwapsinsert_date: TDateTimeField;
    qrySwap: TUniQuery;
    ...
    qryUserFollowersinsert_date: TDateTimeField;
    ...
    qryEventinsert_date: TDateTimeField;
    qryEventevent_date: TDateTimeField;
    qryEventlatitude: TFloatField;
    qryEventlongitude: TFloatField;
    qryEventUsers: TUniQuery;
    ...
    spEventInsert: TUniStoredProc;
    qryEventsList: TUniQuery;
    ...
    qryEventsListinsert_date: TDateTimeField;
    qryEventsListevent_date: TDateTimeField;
    qryEventsListlatitude: TFloatField;
    qryEventsListlongitude: TFloatField;
    spEventDelete: TUniStoredProc;
    spEventAddUser: TUniStoredProc;
    spEventDelUser: TUniStoredProc;
    ...
    spMessageInsert: TUniStoredProc;
    spMessageDelete: TUniStoredProc;
    qrySwapMessagesinsert_date: TDateTimeField;
    qrySwapMessagesuser_image_url: TWideStringField;
    qrySwapMessage: TUniQuery;
    ...
    qrySwapMessageinsert_date: TDateTimeField;
    ...
    qryFeedsevent_date: TDateTimeField;
    ...
    qryFeedsinsert_date: TDateTimeField;
    ...
    qryEditIteminsert_date: TDateTimeField;
    ...
    qryNotificationsevent_date: TDateTimeField;
    ...
    qryNotificationsinsert_date: TDateTimeField;
...
begin
  try
    ...
    begin
      self.Haserrors := true;
      Result := False;
      self.LastErrorMessage := 'user id not set or session expired';
    end;
  Except
    on e: exception do
    begin
      self.Haserrors := true;
      self.LastErrorMessage := 'TdmPgSwap.Notifications Error: ' + e.Message;
      Result := False;
...
begin
  ...
  if not Result then
    try
      ...
      begin
        Result := False;
        self.LastErrorMessage := 'User ID in session not correctly set';
      end;
    except
      on e: exception do
      begin
...
begin
  ...
  begin
    ...
    begin
      Result := False;
      self.LastErrorMessage := 'Session User is not the Item Owner';
    end
    else
    begin
  ...
  begin
    Result := False;
    self.LastErrorMessage := 'Item not Found';
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      self.Haserrors := true;
      Result := False;
      self.LastErrorMessage := 'Item is not owned by Session User';
    end
    else
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      self.LastErrorMessage := 'TdmPgSwap.ItemLike = ' + e.Message + sLineBreak + e.ToString;
    end;
  end;
end;
...
begin
  try
    ...
    begin
      ...
      self.LastErrorMessage := 'TdmPgSwap.ItemInsert= ' + e.Message;
      Result := -1;
    end;
...
begin
  try
    ...
    begin
      Result := False;
      self.LastErrorMessage := 'TdmPgSwap.ItemLikesList ItemID=' + aItemID.ToString + e.Message;
  ...
  try
    ...
    begin
      if qryEditItem.RecordCount = 0 then
        self.LastErrorMessage := 'No Item with ID: ' + aItemID.ToString() + ' Found';
    end;
  except
    on e: exception do
    begin
      Result := False;
      self.LastErrorMessage := 'TdmPgSwap.ItemSearch' + e.Message;
      self.WriteError('TdmPgSwap.ItemSearch', e);
    end;
...
begin
  ...
  try
    ...
    on e: exception do
    begin
      self.LastErrorMessage := 'TdmPgSwap.MessageInsert user_id= ' + self.UserID.ToString + ', swap_id = ' +
...
begin
  ...
  try
    ...
    on e: exception do
    begin
      self.LastErrorMessage := 'TdmPgSwap.MessagesList user_id= ' + self.UserID.ToString + ', swap_id = ' +
...
begin
  ...
  try
    ...
    on e: exception do
    begin
      self.LastErrorMessage := 'TdmPgSwap.MessageWatch aMessageID= ' + aMessageID.ToString + ', swap_id = ' +
        aSwapID.ToString() + e.ToString;
      Result := -1;
    end;
...
begin
  try
    ...
    on e: exception do
    begin
      self.LastErrorMessage := 'TdmPgSwap.NotificationRead user_idid= ' + self.UserID.ToString +
...
begin
  ...
  try
    ...
    if not Result then
    begin
      self.LastErrorMessage := 'No profile found';
      self.Haserrors := true;
    end;
    ...
    begin
      ...
      self.LastErrorMessage := 'TdmPgSwap.OpenSmallProfile=' + e.Message;
    end;
  end;
end;
...
begin
  try
    ...
    begin
      ...
      self.LastErrorMessage := 'TdmPgSwap.SwapperProfile=' + e.ToString;
    end;
  end;
...
begin
  try
    ...
    begin
      Result := False;
      self.LastErrorMessage := 'TdmPgSwap.ProfileInsert: ' + 'User already inserted';
    end;
  except
    on e: exception do
    begin
      self.LastErrorMessage := 'TdmPgSwap.ProfileInsert id= ' + self.UserID.ToString + e.ToString;
      Result := False;
    end;
...
begin
  ...
  try
    ...
    begin
      Result := False;
      self.LastErrorMessage := 'TdmPgSwap.SwapAgree, user_id=' + self.UserID.ToString() + ', swap_id = ' +
...
begin
  ...
  try
    ...
    begin
      Result := False;
      self.LastErrorMessage := 'TdmPgSwap.SwapLis, user_id=' + self.UserID.ToString() + sLineBreak +
        e.Message
    end;
...
begin
  try
    ...
    aProfile.UserProfile := self.UserProfile;

    if aUserID = 0 then
      aID := UserProfile.User.id
    else
      aID := aUserID;
...
begin
  try
    ...
    if Result = 0 then
      self.LastErrorMessage := 'No Followers Found';
  except
    on e: exception do
    begin
      Result := -1;
...
begin
  try
    ...
    begin
      Result := -1;
      self.LastErrorMessage := 'TdmPgSwap.UserLikes, user_id=' + self.UserID.ToString() + sLineBreak +
        e.Message
    end;
...
begin
  ...
  begin
    Result := False;
    self.LastErrorMessage := 'Image not found';
  end
  else if self.qryImageowner_id.AsInteger <> self.UserID then
  begin
    Result := False;
    self.LastErrorMessage := 'User not allowed to delete image'
  end
  else
  begin
