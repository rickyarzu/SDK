...
uses
  ...
  Uni, MemDS, DBAccess, Janua.Core.System,
  // DataModule
  udmJanuaPostgresModel,
  // Janua
  Janua.Core.Entities, Janua.Core.Types, Janua.Core.Functions, Janua.Core.Classes, Janua.Core.Exceptions,
  Janua.Core.Params,
  Datasnap.DBClient, UniProvider, PostgreSQLUniProvider, Janua.Unidac.Connection;
...
type
  TdmPgSystemCommand = class(TdmJanuaPostgresModel)
    ...
    prcSetUserDefaultSchema: TUniStoredProc;
...
begin
  ...
  try
    ...
    Result := (SessionKey <> 'Error') and (SessionKey <> '') and (UserProfile.User.Id > 0);
    {
    ...
    begin
      ...
      self.LastErrorMessage := ('Error Executing Server Procedure TServerMethodsCore.Login: ' + sLineBreak +
        e.Message);
    end;
  end;
end;
...
begin
  try
    ...
    begin
      ...
      self.WriteError(Classname + '.AddUser', e);
    end;
  end;
...
begin
  try
    ...
    begin
      ...
      self.WriteError(Classname + 'ServerFunctions.AddUserSocial', e);
    end;
  end;
end;
...
begin
  ...
  (spUserTest as TUniStoredProc).ParamByName('p_username').AsWideString := aUser.User.Username;
  (spUserTest as TUniStoredProc).ParamByName('p_email').AsWideString := aUser.User.Email;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        self.LastErrorMessage := ('Errore funzione di TServerMethodsCore.Login: ' + sLineBreak + e.Message);
      end;
    end;
  end;
end;
  ...
  try
    try
      ...
      begin
        ...
        begin
          Result := False;
          LastErrorMessage := 'Wrong or nul mail address or Social ID';
          WriteLog('Wrong or nul mail address');
        end
        else if (aUser.SocialID <> '') and (aUser.SocialType = '') then
        begin
          Result := False;
          LastErrorMessage := 'Social ID Without Social Network';
      ...
      begin
        begin
          Result := False;
          LastErrorMessage := 'Attenzione non avete compilato la password';
      ...
      begin
        ...
        begin
          ...
          {
            self.FServerSession := ServerFunctions.GetSession(self.SessionKey, self.qryUserSession, True,
            LastErrorMessage);
          }
        end
        else
...
begin
  ...
  try
    ...
    if aUser.User.SocialID = '' then
      ...
      // aUser.User.Password, True, self.LastErrorMessage, 1, self.FUserKey, self.FRegisterError)
    else
    ...
    // aUser.User.SocialType, aUser.User.Email, True, self.LastErrorMessage, 1, self.FUserKey, self.FRegisterError);

    self.WriteLog('TdmPgSystemCommand.AddUser fine Creazione utente ' + self.LastErrorMessage);
    ...
    begin
      ...
      self.WriteLog('TdmPgSystemCommand.AddUser Risultato OK' + self.LastErrorMessage);
      ...
      begin
        ...
        begin
          ...
          self.prcSetUserDefaultSchema.ParamByName('p_session_key').AsWideString := self.SessionKey;
      ...
      begin
        ...
        self.WriteLog('TdmPgSystemCommand.AddUser Risultato: ' + self.LastErrorMessage);
      end;
    end;
  except
    on e: Exception do
...
begin
  ...
  try
    ...
    begin
      ...
      LastErrorMessage := e.Message;
      FRegisterError := TJanuaRegisterError.jreSystemError;
      self.WriteError(Classname + '.AdminAddUser', e);
    end;
  end;
...
begin
  ...
  try
    spUserPayment.ParamByName('p_user_id').AsInteger := aUserID;
    spUserPayment.ParamByName('p_schema_id').AsInteger := 0;
    spUserPayment.ParamByName('p_amount').AsCurrency := aAmount;
    spUserPayment.ParamByName('p_interval_type').AsWideString := 'Y';
    spUserPayment.ParamByName('p_interval_duration').AsSmallInt := 1;
    spUserPayment.ParamByName('p_payment_date').AsDate := aDate;
    spUserPayment.Execute;
    aUser := self.UserProfile.User;
    aUser.PaymentExpiration := self.spUserPayment.ParamByName('result').AsDate;
    UserProfile.User := aUser;
    Result := True;
  except
    on e: Exception do
    begin
...
begin
  ...
  try
    ...
    on e: Exception do
    begin
      self.WriteError(Classname + '.FindUserByKey', e);
    end;
  end;

  {
    (const aKey: string; aQuery: TUniQuery; var aUser: TJanuaRecordUserProfile)
...
begin
  try
    ...
    qryUserSession.ParamByName('db_session_key').AsString := self.SessionKey;
    OpenDataset(qryUserSession);
    self.ServerSession.LoadFromDataset(qryUserSession);
    ...
    begin
      qryUserSession.Close;
      qryUserSession.Connection.Disconnect;
      WriteError(Classname + '.GetSession', e);
    end;
  end;
end;
...
begin
  if self.SessionKey = '' then
    self.SessionKey := self.ServerSession.key;

  qrySearchUsers.ParamByName('session_key').Value := self.SessionKey;
end;
...
begin
  qrySearchUserProfile.Close;
  qrySearchUserProfile.ParamByName('p_search').AsString := '%';
  qrySearchUserProfile.ParamByName('p_user_id').AsInteger := qrySearchUsersdb_user_id.AsInteger;
  ServerFunctions.OpenDataset(qrySearchUserProfile);
end;
...
begin
  ...
  self.ServerSession.key := self.spStartSessionByUrl.ParamByName('result').AsWideString;
  if self.ServerSession.key = '' then
  begin
    self.qrySessionByUrl.Close;
    qrySessionByUrl.ParamByName('p_url').AsWideString := aUrl;
    qrySessionByUrl.Open;
    ServerSession.key := qrySessionByUrlstartsessionbyurl.AsWideString
  end;
  SetSessionKey(self.ServerSession.key);
