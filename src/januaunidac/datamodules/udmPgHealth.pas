...
uses
  ...
  // Devart
  Uni, Data.DB, MemDS, DBAccess, UniProvider, PostgreSQLUniProvider,
  // Datasnap
  Datasnap.Provider, Datasnap.DBClient,
  // DataModules
  udmJanuaPostgresModel,
  // Janua
  Janua.Core.Types, Janua.Core.Health, Janua.Core.System, Janua.Core.Functions,
  Janua.Core.JPublic, Janua.Core.Classes, Janua.Core.Entities, Janua.Unidac.Connection;

type
  TdmPgHealth = class(TdmJanuaPostgresModel)
    dsBranches: TDataSource;
    tbBranches: TUniTable;
    tbBranchSpecialities: TUniQuery;
    ...
    prcCreateNewDoctor: TUniStoredProc;
    tbLocalOrganizations: TUniTable;
    ...
    tbHealthProfile: TUniQuery;
    qryAssociations: TUniQuery;
    ...
    tbBranchesinsert_date: TDateTimeField;
    tbBranchesupdate_date: TDateTimeField;
    tbBranchSpecialitiesinsert_date: TDateTimeField;
    tbBranchSpecialitiesupdate_date: TDateTimeField;
    cdsBranchesbranch_id: TSmallintField;
    cdsBranchesbranch_code: TWideStringField;
    cdsBranchesbranch_name: TWideStringField;
    cdsBranchesinsert_date: TDateTimeField;
    cdsBranchesupdate_date: TDateTimeField;
    ...
    qrySpecialities: TUniQuery;
    dsSpecialities: TDataSource;
    qrySubSpecialities: TUniQuery;
    ...
    cdsBranchSpecialitiesinsert_date: TDateTimeField;
    cdsBranchSpecialitiesupdate_date: TDateTimeField;
    dsCouncils: TDataSource;
    dsHealthRegions: TDataSource;
    dsLocalOranizations: TDataSource;
    qrySearchDoctor: TUniQuery;
  ...
  public
    ...
    function AddNotification(const aUser: TJanuaRecordUserProfile;
...
begin
  ...
  begin
    ...
    // una volta filtrata la branca occorre andare a posizionare la 'specialit?'

    self.tbBranchSpecialities.Close;
    GetCustomServerFunctions.OpenDataset(self.tbBranchSpecialities);
...
begin
  try
    ...
    begin
      ...
      Result := False;
      WriteError(' errore registrazione profilo medico ', e, 'TdmHealth.AddDoctorProfile:');
    end;
  end;
end;
...
begin
  ...
  GetCustomServerFunctions.OpenDataset(qrySearchDoctor);
...
begin
  ...
  self.GetCustomServerFunctions.OpenDataset(tbHealthProfile);
...
begin
  ...
  try
    ...
    GetCustomServerFunctions.OpenDataset(self.tbHealthProfile);
    ...
    begin
      ...
      self.GetCustomServerFunctions.OpenDataset(qrySearchUserProfile);
    end;
    ...
    begin
      Result := False;
      SetHaserrors(True);
      self.LastErrorMessage := 'TdmPgHealth.SearchUserProfilebyID: ' + ID.ToString + sLineBreak + 'Error: ' +
        e.Message;
    end;
...
begin
  self.qryNotifications.ParamByName('db_user_id').AsInteger := self.GetUserProfile.User.ID;
  self.GetCustomServerFunctions.OpenDataset(qryNotifications);
...
begin
  ...
  {
    self.FCustomServerFunctions.OpenDataset(tbRegions);
    tbRegions.Last;
    ...
    self.FCustomServerFunctions.OpenDataset(tbLocalOrganizations);
...
begin
  GetCustomServerFunctions.OpenDataset(tbBranchSpecialities);
end;
