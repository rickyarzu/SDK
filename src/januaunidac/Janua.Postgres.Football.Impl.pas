...
uses Janua.Football.Server.Impl, Janua.Orm.Intf, Janua.Football.Intf, Janua.Football.Server.Intf,
  Janua.Core.DB.Intf, Janua.Server.Intf, udmPgFootballStorage, Uni;
...
type
  ...
  private
    function GetParamByName(const aName: string; aDataset: TUniQuery): TUniParam;
  protected
...
uses System.SysUtils, Spring, Janua.Orm.UniDac, Janua.Core.Functions, System.Math, System.StrUtils,
  Janua.Football.Impl, Janua.Orm.Types, Janua.Application.Framework;
...
begin
  ...
  if (FDM.qryLastMatch.ParamByName('season_id').Value <> self.FFootballSettings.season_id) or
    (FDM.qryLastMatch.ParamByName('league_id').Value <> FFootballSettings.league_id) or (FFootballSettings.match_day = 0)
  then
  begin
...
function TJanuaPgFootballStorage.GetParamByName(const aName: string; aDataset: TUniQuery): TUniParam;
begin
...
begin
  ...
  if (FDM.qryGolRanking.Params[0].AsInteger <> aSeasonID) or (FDM.qryGolRanking.Params[1].AsInteger <> aLeague) then
...
begin
  ...
  if (FDM.qryGolRanking.ParamByName('season_id').Value <> FFootballSettings.season_id) or
    (FDM.qryGolRanking.ParamByName('league_id').Value <> FFootballSettings.league_id) or (not FDM.qryGolRanking.Active)
  then
  begin
    FDM.qryGolRanking.Close;
...
begin
  (*
    ...
    Live score da sistema. Se Match ID ? ?nullo? apre nell?ordine
    Campionato (stagione lega ultima giornata)
    Classifica Squadre
    Classifica Marcatori
    se MatchDay = 0 esegue GetLastMatchDay

    dopodich? apre i risultati se match_id ? <> 0 allora apre il match altrimenti apre tutti i match della giornata insieme.
...
begin
  ...
  OpenChampionship;
  // Se non ? gi? aperta la query e non ritorna un numero di record > 0 allora rieseguo la ricerca
...
begin
  ...
  if (FDM.qryMatches.ParamByName('season_id').Value <> FFootballSettings.season_id) or
    ...
    (FDM.qryMatches.ParamByName('match_day_number').Value <> FFootballSettings.match_day) or (not FDM.qryMatches.Active)
  then
  begin
...
begin
  try
    ...
    on e: exception do
      RaiseException('OpenPlayerCareer', e, self, self.LogString);
  end
end;
...
begin
  ...
  try
    ...
    on e: exception do
      RaiseException('OpenSeason', e, self, self.LogString);
  end;
end;
...
begin
  ...
  with FDM do
    ...
    begin
      ...
      FFootballSettings.match_day := IfThen(qryLastMatchlast_match.AsInteger = 0, 1, qryLastMatchlast_match.AsInteger);
      qryLastMatch.Close;
    end;
  Result := FFootballSettings.match_day;
end;

procedure TJanuaPgFootballStorage.SetAfterCreateDataset;
begin
  inherited;

end;

procedure TJanuaPgFootballStorage.SetAfterDestroyDataset;
begin
  inherited;

end;

procedure TJanuaPgFootballStorage.SetInternalCreateDataset;
begin
  inherited;
...
procedure TJanuaPgFootballStorage.SetInternalCreateDBDataSets;
begin
  inherited;
  ...
  FInternalCreateDBDataSets := procedure
    begin
      ...
      FjdsMatches := TJanuaPgDataset.Create(FDM.qryMatches, [TJanuaEntity.FtCalendar]);
      FjdsNextMatchDay := TJanuaPgDataset.Create(FDM.qryNextMatchDay, [TJanuaEntity.FtMatchDays]);
      jdsChampionship := TJanuaPgDataset.Create(FDM.qryChampionship, [TJanuaEntity.FtChampionships]);
      jdsChampionships := TJanuaPgDataset.Create(FDM.qryChampionships, [TJanuaEntity.FtChampionships]);
      jdsGolRanking := TJanuaPgDataset.Create(FDM.qryGolRanking, [TJanuaEntity.FtGoalRanking]);
      jdsChampRanking := TJanuaPgDataset.Create(FDM.qryChampRanking, [TJanuaEntity.FtRankingTable]);
      jdsSeasons := TJanuaPgDataset.Create(FDM.qrySeasons, [TJanuaEntity.FtSeasons]);
      // per ora uso la stessa tabelle di jdsChampionships
      jdsSeasonLeagues := TJanuaPgDataset.Create(FDM.qryChampionships, [TJanuaEntity.FtChampionships]);
      jdsMatchDay := TJanuaPgDataset.Create(FDM.qryMatchDay, [TJanuaEntity.FtMatchDays]);
      jdsNextMatches := TJanuaPgDataset.Create(FDM.qryNextMatches, [FtCalendar, FtTeamsHome, FtTeamsVisitors]);
      // le singole partite con match, formazioni ed eventi del match
      FjdsMatchDetail := TJanuaPgDataset.Create(FDM.qryMatchDetail, [FtCalendar, FtTeamsHome, FtTeamsVisitors]);
      FjdsMatchDetailEvents := TJanuaPgDataset.Create(FDM.qryMatchDetailEvents, [FtMatchEvents]);
      FjdsMatchDetailHomePlayers := TJanuaPgDataset.Create(FDM.qryMatchHomePlayers, [FtPlayers]);
      FjdsMatchDetailVisitorPlayers := TJanuaPgDataset.Create(FDM.qryMatchVisitorsPlayers, [FtPlayers]);
      // Dynamic Ranking
      FjdsDynamicRanking := TJanuaPgDataset.Create(FDM.qryDynamicRanking, [TJanuaEntity.FtRankingTable]);
      FjdsLookupMatchDays := TJanuaPgDataset.Create(FDM.lkpMatchDays, [TJanuaEntity.FtMatchDays]);
      // in questo momento Goal Ranking non ? proprio 'molto' dynamic ...........
      { TODO : Verificare FjdsDynamicGoalRanking non hanno il Dataset Assegnato }
      FjdsDynamicGoalRanking := TJanuaPgDataset.Create(FDM.qryGolRanking, [TJanuaEntity.FtGoalRanking]);
      FjdsPlayerCareers := TJanuaPgDataset.Create(FDM.qryPlayerCareer, [TJanuaEntity.FtPlayerCareer]);
      // Calendar Days & Calendar Matches
      // football.ft_calendar c, football.ft_teams a, football.ft_teams b
      FjdsCalendarMatches := TJanuaPgDataset.Create(FDM.qryCalendarMatches, [TJanuaEntity.FtCalendar, FtTeamsHome,
        FtTeamsVisitors]);
      // football.ft_match_days m, football.ft_leagues l, football.ft_seasons s
      FjdsCalendarMatchDay := TJanuaPgDataset.Create(FDM.qryCalendarDay, [FtMatchDays, FtSeasons, FtLeagues]);
      ...
      FjdsSelectedSeason := TJanuaPgDataset.Create(FDM.qrySeasons, [FtSeasons]);
    end;

end;

procedure TJanuaPgFootballStorage.SetInternalDestroyDataset;
begin
  inherited;
...
begin
  ...
  if (FDM.qryChampionship.ParamByName('season_id').AsInteger <> FFootballSettings.season_id) or
    (FDM.qryChampionship.ParamByName('league_id').AsInteger <> FFootballSettings.league_id) or not FDM.qryChampionship.Active
  then
  begin
