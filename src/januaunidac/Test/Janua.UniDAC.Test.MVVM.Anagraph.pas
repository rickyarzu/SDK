unit Janua.UniDAC.Test.MVVM.Anagraph;
...
uses System.SysUtils, System.StrUtils, Janua.Mock.Firedac.Framework, Janua.Application.Framework,
  System.Types, Janua.Uni.Framework;
...
begin
  ...
  Assert.IsNotNull(lModel.jdsDataset, 'jdsDataset');
  Assert.IsNotNull(lModel.jdsDetail, 'jdsDetail');
  Assert.IsNotNull(lModel.jdsRecordDataset, 'jdsRecordDataset');
  if aPostgres then
  begin
    lModel.GroupID := 10;
  ...
  lModel.Last(nil);
  lGUID := lModel.jdsAnagraphs.GUID;
  Assert.IsNotNull(lModel.AnagraphRecord, 'AnagraphRecord');
  Assert.IsNotNull(lModel.CurrentRecord, 'AnagraphRecord');
  lModel.LoadRecord;
  lModel.CurrentRecord.LoadRecord(True);
  Assert.AreEqual(lGUID, lModel.CurrentRecord.GUID, 'GUID Dataset <> GUID Record');
...
begin
  ...
  begin
    ...
    Assert.AreEqual(38958, FViewModel.dsMaster.Dataset.RecordCount)
  end
  else
