...
uses
  ...
  // Data Modules
  Janua.Core.DB.Intf, udmPgCmsStorage, udmPgCms, udmPgCmsGraphicMenu, JOrm.Cms.Articles.Intf;
...
type
  ...
  TJanuaServerPgCms = class(TJanuaCustomServerCms)
  private
    FCms: TCms;
    FFeeds: TJanuaFeeds;
    FLastSearch: string;
...
uses
  JOrm.Cms.MainArguments.Impl, JOrm.Cms.MainArguments.Intf, Janua.Core.Servers;
...
begin
  if not assigned(self.FServerSystem) then
    self.LocalRaiseException('Select System Component First!!');
  Result := True;
  if Verbose then
...
begin
  ...
  if not supports(lCms, IJanuaServerDataModule, FIDM) then
    raise exception.Create('lCms do not supports IJanuaServerDataModule');

  FConnected := lCms.TestConnect;
end;
...
function TJanuaServerPgCms.InternalActivate: boolean;
begin
  Result := inherited;
  try
    if Result then
...
procedure TJanuaServerPgCms.InternalDeactivate;
begin
...
function TJanuaServerPgCms.SearchArticle(ricerca: string; maxarticles: smallint; language: smallint): smallint;
begin
  try
    try
      if self.Verbose then
