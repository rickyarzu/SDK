...
uses
  // Delphi
  System.Classes, System.SysUtils, FMX.Types, FMX.Controls, FMX.Forms, FMX.WebBrowser, Janua.Core.Classes;
...
{$IFDEF UNICODE}
    procedure SaveToStream(const Stm: TStream; const Encoding: TEncoding); overload;
{$ENDIF}
    procedure SaveToFile(const FileName: string); overload;
{$IFDEF UNICODE}
    procedure SaveToFile(const FileName: string; const Encoding: TEncoding); overload;
...
procedure StringToStreamBOM(const S: string; const Stm: TStream; const Encoding: TEncoding);
var
  Bytes: TBytes;
  Preamble: TBytes;
begin
...
procedure TWebBrowserWrapper.InternalSaveDocumentToStream(const Stream: TStream);
var
  StreamAdapter: IStream;
  PersistStreamInit: IPersistStreamInit;
begin
...
procedure TWebBrowserWrapper.LoadFromString(const HTML: string; const Encoding: TEncoding);
var
...
procedure TWebBrowserWrapper.NavigateToResource(const Module: HMODULE; const ResName, ResType: PChar);
begin
  NavigateToURL(MakeResourceURL(Module, ResName, ResType));
end;

procedure TWebBrowserWrapper.NavigateToResource(const ModuleName: string; const ResName, ResType: PChar);
begin
...
begin
  ...
  Flags := navNoHistory;
  if AnsiStartsText('res://', URL) or AnsiStartsText('file://', URL) or AnsiStartsText('about:', URL) or
    AnsiStartsText('javascript:', URL) or AnsiStartsText('mailto:', URL) then
...
procedure TWebBrowserWrapper.SaveToFile(const FileName: string; const Encoding: TEncoding);
var
  FileStream: TFileStream;
begin
  FileStream := TFileStream.Create(FileName, fmCreate);
...
procedure TWebBrowserWrapper.SaveToStream(const Stm: TStream; const Encoding: TEncoding);
var
  HTML: string;
begin
  HTML := SaveToString;
