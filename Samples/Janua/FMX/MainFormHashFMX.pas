...
type
  TFormMain = class(TForm)
    ...
    procedure EditInputKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
...
begin
  ...
  begin
    // Find the class type of the selected formatting class
    var
    aTest := ComboBoxOutputFormatting.Items[ComboBoxOutputFormatting.ItemIndex];
    OutputFormatting := TDECFormat.ClassByName(aTest);
  end
  else
  begin
    ShowErrorMessage('No input format selected');
  ...
  begin
    ...
    begin
      ...
      if (Rounds >= RoundsInterf.GetMinRounds) and (Rounds <= RoundsInterf.GetMaxRounds) then
    ...
    try
      ...
      if (not Assigned(ExtensibleInterf)) and (not Assigned(LastByteLengthInterf)) and
...
begin
{$IF RTLVersion > 30}
  if TPlatformServices.Current.SupportsPlatformService(IFMXDialogServiceASync, IInterface(AsyncDlg)) then
    AsyncDlg.MessageDialogAsync(Translate(ErrorMsg), TMsgDlgType.mtError, [TMsgDlgBtn.mbOk],
      TMsgDlgBtn.mbOk, 0,
      procedure(const AResult: TModalResult)
      begin
      end);
{$ELSE}
  MessageDlg(Translate(ErrorMsg), TMsgDlgType.mtError, [TMsgDlgBtn.mbOk], 0);
{$ENDIF}
end;

procedure TFormMain.VertScrollBox1CalcContentBounds(Sender: TObject; var ContentBounds: TRectF);
begin
...
begin
  ...
  CheckBoxIsPasswordHash.IsChecked := HashClass.IsPasswordHash;
  ...
  else if (ClientHeight < LayoutHeight) and (LayoutHeight < Screen.Height) then
...
begin
  ...
  begin
    ...
    if (Cost < MinCost) or (Cost > MaxCost) then
      ShowErrorMessage(Format('Invalid input. Cost must be between %0:d and %1:d', [MinCost, MaxCost]));
...
procedure TFormMain.EditInputKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
...
begin
  var
  sTest := ComboBoxHashFunction.Items[ComboBoxHashFunction.ItemIndex];
  Result := 'THash_' + sTest;
end;
...
begin
  ...
  var
  aTest := ComboBoxInputFormatting.Items[ComboBoxInputFormatting.ItemIndex];
  Result := TDECFormat.ClassByName(aTest);
end;
