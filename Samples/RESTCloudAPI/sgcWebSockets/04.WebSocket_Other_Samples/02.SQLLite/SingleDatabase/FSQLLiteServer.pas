...
uses
  ...
  sgcWebSocket_Protocol_Dataset_Server, sgcWebSocket_Server, sgcJSON;

type
  TFRMSQLLiteServer = class(TForm)
    DBGrid1: TDBGrid;
    ...
    sgcWSPServer_Dataset1: TsgcWSPServer_Dataset;
...
var
  FRMSQLLiteServer: TFRMSQLLiteServer;

implementation

{$R *.dfm}

procedure TFRMSQLLiteServer.FormCreate(Sender: TObject);
begin
  txtDatabase.Text := ExtractFilePath(Application.ExeName) + 'Employees.s3db';
end;

procedure TFRMSQLLiteServer.btnOpenDBClick(Sender: TObject);
begin
...
procedure TFRMSQLLiteServer.FDTable1AfterOpen(DataSet: TDataSet);
begin
