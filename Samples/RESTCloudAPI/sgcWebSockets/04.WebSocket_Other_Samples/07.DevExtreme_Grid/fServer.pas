...
uses
  ...
  sgcWebSocket_Protocol_Base_Server, sgcWebSocket_Classes_Indy, sgcBase_Classes,
...
begin
  {$IFDEF UNICODE}
    ShellExecute(Application.Handle, 'open', PWideChar(aBrowserName),
      PWideChar(ExtractFilePath(Application.ExeName) + 'index.html'), '', 0);
  {$ELSE}
    ShellExecute(Application.Handle, 'open', PAnsiChar(aBrowserName),
      PAnsiChar(ExtractFilePath(Application.ExeName) + 'index.html'), '', 0);
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FieldByName('EXCHANGE').AsString := 'Exchange';

      FieldByName('OPEN').AsFloat := RandomRange(8300, 8320) + j;
      FieldByName('HIGH').AsFloat := RandomRange(8320, 8400) + j;
      FieldByName('LOW').AsFloat := RandomRange(8200, 8300) + j;
      FieldByName('LAST').AsFloat := RandomRange(8200, 8400) + j;
      Post;
    end;
  end;
end;
