...
begin
  ...
  else if StartsWith('UPDATE ', SqlUp) or StartsWith('DELETE ', SqlUp) or StartsWith('INSERT ', SqlUp) then
  begin
    ExecuteUpdate(Sql);
  end
  else
...
begin
  ...
  begin
    ...
    if (InputValue = 'TRUE') or (InputValue = 'Y') then
    begin
      Result := 'TRUE';
      Exit;
    end
    else if (InputValue = 'FALSE') or (InputValue = 'N') then
    begin
      Result := 'FALSE';
      Exit;
    end
...
begin
  ...
  AValue := 'rickyarzu-facilitator@gmail.com'; // ReadStringParameter('User');
  Params.Add(S_FD_ConnParam_Common_UserName, AValue);

  AValue := 'Afyvz0RNwrvEOyjl5MG_1j7G386kvA1pmgpRzULeLYyay8cWO3-MEXGk9i1EgKOCmOf0MySJuvedli8s';//ReadStringParameter('Password');
  Params.Add('Password', AValue);

  AValue := 'END716khABWCoF8YgQVJ3zBX8xVmHmkl0XLkNipfG-eDWzAIs00JJ_JNcgS4WSffQYWrrTwn4GWeUr7h'; //ReadStringParameter('Signature');
  Params.Add('Signature', AValue);

  AValue := ReadBoolParameter('UseSandbox');
  Params.Add('UseSandbox', AValue);

  AValue := ReadStringParameter('Schema');
  Params.Add('Schema', AValue);

end;

begin
  FDLink := TFDPhysCDataPayPalDriverLink.Create(nil);
  try
