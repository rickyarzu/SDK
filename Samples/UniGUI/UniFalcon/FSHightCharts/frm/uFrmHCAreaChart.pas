...
type
  TfrmHCAreaChart = class(TUniFrame)
    CDS: TClientDataSet;
    pnlTop: TUniPanel;
...
procedure TfrmHCAreaChart.btnLoadClick(Sender: TObject);
begin
  HighCharts.LoadChart;
end;

procedure TfrmHCAreaChart.lblInfoClick(Sender: TObject);
begin
  UniSession.AddJS('window.open("'+TUniLabel(Sender).Caption+'")');
end;

procedure TfrmHCAreaChart.LoadData;
begin
  with CDS do
  begin
    Close;
...
procedure TfrmHCAreaChart.UniFrameCreate(Sender: TObject);
begin
  LoadData;
  HighCharts.LoadChart;
end;
