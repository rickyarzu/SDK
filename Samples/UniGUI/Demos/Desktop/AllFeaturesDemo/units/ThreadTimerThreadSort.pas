// Thread Sort = Thread Timer
unit ThreadTimerThreadSort;
...
type
  TUniThreadTimerThreadSort = class(TUniFrame)
...
procedure TUniThreadTimerThreadSort.UniButton1Click(Sender: TObject);
begin
  if FBusy then Exit;
...
procedure TUniThreadTimerThreadSort.UniButton2Click(Sender: TObject);
begin
  UniButton2.Enabled := False;
  FAbort := True;
end;

procedure TUniThreadTimerThreadSort.UniFrameDestroy(Sender: TObject);
begin
...
procedure TUniThreadTimerThreadSort.UniFrameReady(Sender: TObject);
begin
  if UniApplication.Parameters.Values['auto_start'] = '1' then
  begin
    UniApplication.Parameters.Values['auto_start'] := ''; // don't need it anymore
    UniButton1Click(nil);
  end;
end;

procedure TUniThreadTimerThreadSort.ThreadTimerTimer(Sender: TObject);
begin
  try
    case FSortMethod of
...
procedure TUniThreadTimerThreadSort.UniTimer1Timer(Sender: TObject);
begin
  ThreadTimer.Lock;
  try
...
procedure TUniThreadTimerThreadSort.InitArray(ASize: Integer);
var
  I : Integer;
begin
...
procedure TUniThreadTimerThreadSort.UpdateSeries;
var
  I, M : Integer;
begin
...
procedure TUniThreadTimerThreadSort.QuickSort(var A: array of Integer);
  ...
  begin
    ...
    ThreadTimer.Lock; // Lock thread when accessing shared variables
                      ...
                      // We put it to demonstrate locking )
    try
      if FAbort then Exit;
    finally
      ThreadTimer.Release;
...
procedure TUniThreadTimerThreadSort.BubbleSort(var A : array of Integer);
var
  I, J, T: Integer;
begin
...
initialization
  RegisterClass(TUniThreadTimerThreadSort);
