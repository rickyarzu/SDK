...
uses
  ...
  uniGUIClasses, uniGUIFrame, uniBasicGrid, uniDBGrid, uniSpeedButton,
  uniSpinEdit, uniTrackBar, uniButton, uniBitBtn, uniPanel, uniHTMLFrame,
  uniGUIBaseClasses, DB, DBClient, uniTimer, uniLabel;

type
  TUniMapLayout = class(TUniFrame)
...
{$R *.dfm}
 ...
 procedure TUniMapLayout.ClientDataSet1AfterScroll(DataSet: TDataSet);
begin
  if DataCreated then
    SetCoord(DataSet.FieldByName('Latitude').AsFloat, DataSet.FieldByName('Longitude').AsFloat);
end;

procedure TUniMapLayout.SetCoord(Lat, Long: Real);
begin
  if WebMode then
...
procedure TUniMapLayout.UniFrameCreate(Sender: TObject);
begin
FFirst := True;
...
procedure TUniMapLayout.UniTimer1Timer(Sender: TObject);
begin
if FFirst then
  begin
