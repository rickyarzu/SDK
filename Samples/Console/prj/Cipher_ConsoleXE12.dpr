...
begin
  ...
  try
    try
      ...
      Tmp := EncodeString64(System.SysUtils.StringOf(Output));
      Write(Tmp);
      WriteLn;

      // Back to Bytes
      Input := System.SysUtils.BytesOf(DecodeString64(Tmp));
      ...
      Tmp := RawByteString(System.SysUtils.StringOf(Output));

      WriteLn('Decrypted with different key: ' + Tmp);

      ReadLn;

      Tmp := Janua.Core.Functions.EncryptDES3(SourceText);
      WriteLn('Encrypted with Janua API key: ' + Tmp);
      SourceText := Janua.Core.Functions.DecryptDES3(Tmp);
      WriteLn('Encrypted with Janua API key: ' + SourceText);
      Readln;
