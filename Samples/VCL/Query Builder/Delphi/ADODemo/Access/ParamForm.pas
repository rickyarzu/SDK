...
begin
  ...
  if raNull.Checked then
    FParam.VarValue := NULL
  else
  if raValue.Checked then
  begin
    try
      case FParam.DataType of
        ftSmallint, ftWord, ftLargeint, ftInteger:
          FParam.VarValue := StrToInt(edValue.Text);
        ftBoolean: ;
        ftFloat, ftCurrency, ftBCD:
          FParam.VarValue := StrToFloat(edValue.Text);
        ftDate:
          FParam.VarValue := StrToDate(edValue.Text);
        ftTime:
          FParam.VarValue := StrToTime(edValue.Text);
        ftDateTime:
          FParam.VarValue := StrToDateTime(edValue.Text);
        else
...
begin
  ...
  if VarType(FParam.VarValue) = varNull then
  begin
