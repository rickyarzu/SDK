...
uses Windows, System.SysUtils, VCL.Forms, System.Types, VCL.Dialogs, VCL.StdCtrls, VCL.ExtCtrls, VCL.Controls,
  System.Classes, QBWindow, QBParser, QBFloatPanel;
...
type
  ...
  public
    property QueryBuilder: TFullQueryBuilder read FQueryBuilder write SetQueryBuilder;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cbKeywordsFormat.ItemIndex := Ord(FQueryBuilder.ScriptFormat.KeywordsFormat);
      cbFunctionsFormat.ItemIndex := Ord(FQueryBuilder.ScriptFormat.FunctionsFormat);
      cbBracketAllConditions.Checked := FQueryBuilder.ScriptFormat.AlwaysBracketConditions;
    end;
  end;
end;

procedure TfmQBOptions.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  if Assigned(FQueryBuilder) and (ModalResult = mrOk) then
  begin
    if chRowSelect.Checked then
      FQueryBuilder.Options := FQueryBuilder.Options + [qboRowSelect];
    if chDragFieldName.Checked then
      FQueryBuilder.Options := FQueryBuilder.Options + [qboDragFieldName];
    if chHideSelection.Checked then
      FQueryBuilder.Options := FQueryBuilder.Options + [qboHideSelection];
    if chShowFieldTypes.Checked then
      FQueryBuilder.Options := FQueryBuilder.Options + [qboShowFieldTypes];
    if chXPTablesStyle.Checked then
      FQueryBuilder.Style.TableStyle := qtsXPStyle
    else
      FQueryBuilder.Style.TableStyle := qtsStandard;
    ...
    FQueryBuilder.ScriptFormat.AlwaysBracketConditions := cbBracketAllConditions.Checked;

    if chCriterions.Checked then
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs + [qbtCriterions]
    else
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs - [qbtCriterions];

    if chSelection.Checked then
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs + [qbtSelection]
    else
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs - [qbtSelection];
    if chGroupCriterions.Checked then
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs + [qbtGroupCriterions]
    else
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs - [qbtGroupCriterions];
    if chSorting.Checked then
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs + [qbtSorting]
    else
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs - [qbtSorting];
    if chData.Checked then
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs + [qbtData]
    else
      FQueryBuilder.VisibleTabs := FQueryBuilder.VisibleTabs - [qbtData];
  end;
end;
...
begin
  ...
  begin
    ...
    TPaintBox(Sender).Canvas.Polygon([Point(1, 1), Point(1, TPaintBox(Sender).Height - 2),
