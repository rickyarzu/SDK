...
type
  TfmMain = class(TForm)
    ...
    QueryBuilder: TPgSQLQueryBuilder;
    miSeparator4: TMenuItem;
    miSystemTables: TMenuItem;
    ...
    procedure miSystemTablesClick(Sender: TObject);
...
resourcestring
  sCaption = 'EMS PostgreSQL Query Builder demo application [%s]';
...
begin
  ...
  try
    ...
    Query.SQL.Text := Format(sSelect,
      [FormatByQuotes(SelLI.Caption, True)]);
    Query.Open();
    try
...
begin
  if Database.Connected then
    Caption := Format(sCaption, [Database.Database])
  else
...
begin
  if ShowConnectForm(Database) then
    UpdateCaption();
end;
...
begin
  ...
  Q.Connection := Database;
  try
    Q.Sql.Text := 'SELECT VERSION()';
    Q.Open();
    try
      try
        Ver := StrToFloat(StringReplace(Copy(Q.Fields[0].AsString, 12, 3), '.', ',', []));
      except
        Ver := 0;
      end;
    finally
    ...
    QueryBuilder.UseNamespaces := Ver >= 7.3;
    if Ver >= 7.3 then
    begin
      Q.Sql.Text := 'SELECT b.nspname, a.relname ' +
        'FROM pg_class a, pg_namespace b ' +
        'WHERE (a.relnamespace = b.oid) AND ' +
        '(a.relkind = ''r'') ';
      if not miSystemTables.Checked then
        Q.Sql.Text := Q.Sql.Text + 'AND (SUBSTR(a.relname, 1, 3) <> ''pg_'') ';
      Q.Sql.Text := Q.Sql.Text + 'ORDER BY b.nspname, a.relname';
    end
    else
    begin
      Q.Sql.Text := 'SELECT pg_class.relname ' +
        'FROM pg_class ' +
        'WHERE (pg_class.relkind = ''r'') ';
      if not miSystemTables.Checked then
        Q.Sql.Text := Q.Sql.Text + 'AND (SUBSTR(pg_class.relname, 1, 3) <> ''pg_'') ';
      Q.Sql.Text := Q.Sql.Text + 'ORDER BY pg_class.relname';
    end;
    Q.Open();
    try
      Q.First();
      ...
      try
        ...
        begin
          ...
          if Ver >= 7.3 then
            NewItem.Caption := Q.Fields[0].AsString + '.' + Q.Fields[1].AsString
          else
            NewItem.Caption := Q.Fields[0].AsString;
...
begin
  ...
  begin
    ...
    if I > 1 then
    begin
      id1 := Copy(Result, 1, I - 1);
      if id1[1] <> '"' then
        id1 := '"' + id1 + '"';
      id2 := Copy(Result, I + 1, Length(Result));
      if id2[1] <> '"' then
        id2 := '"' + id2 + '"';
      Result := Format('%s.%s', [id1, id2])
    end
...
procedure TfmMain.miSystemTablesClick(Sender: TObject);
begin
  miSystemTables.Checked := not miSystemTables.Checked;
  RefreshTableList();
end;
