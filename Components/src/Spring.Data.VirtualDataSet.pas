...
{$ENDIF}
  ...
  TCustomSpringVirtualDataSet = class;

  TChangeRecordEvent = procedure(Sender: TCustomSpringVirtualDataSet; Index: Integer) of object;
  TGetFieldValueEvent = procedure(Sender: TCustomSpringVirtualDataSet;
    Field: TField; Index: Integer; var Value: Variant) of object;
  TGetRecordCountEvent = procedure(Sender: TCustomSpringVirtualDataSet; var Count: Integer) of object;

  TCustomSpringVirtualDataSet = class(TDataSet)
  private
    fRowBufSize: Integer;
  ...
  protected
    type
      ...
      strict private
        fField: TBlobField;
        fDataSet: TCustomSpringVirtualDataSet;
        fFieldNo: Integer;
        fModified: Boolean;
        fData: Variant;
        fFieldData: Variant;
  ...
  TVirtualDataSet = class(TCustomSpringVirtualDataSet)
  published
    property Active;
    property Filtered;
    property ReadOnly;
...
{$REGION 'TCustomSpringVirtualDataSet'}

constructor TCustomSpringVirtualDataSet.Create(AOwner: TComponent);
var
...
destructor TCustomSpringVirtualDataSet.Destroy;
begin
  inherited Destroy;
  fIndexList.Free;
end;

function TCustomSpringVirtualDataSet.AllocRecordBuffer: TRecordBuffer;
begin
...
{$IFDEF NEXTGEN}
function TCustomSpringVirtualDataSet.AllocRecBuf: TRecBuf;
begin
  Result := AllocRecordBuffer;
end;
{$ENDIF}

function TCustomSpringVirtualDataSet.BookmarkValid(Bookmark: TBookmark): Boolean;
begin
  Result := Assigned(Bookmark) and Assigned(PObject(Bookmark)^)
    and fIndexList.Contains(PObject(Bookmark)^);
end;

function TCustomSpringVirtualDataSet.CompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
const
...
function TCustomSpringVirtualDataSet.CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream;
begin
  Result := TBlobStream.Create(Field as TBlobField, Mode);
end;

procedure TCustomSpringVirtualDataSet.DataEvent(Event: TDataEvent;
...
procedure TCustomSpringVirtualDataSet.DoDeleteRecord(Index: Integer);
begin
  if Assigned(fOnDeleteRecord) then
    fOnDeleteRecord(Self, Index);
end;

procedure TCustomSpringVirtualDataSet.DoFilterRecord(var Accept: Boolean);
begin
  OnFilterRecord(Self, Accept);
end;

procedure TCustomSpringVirtualDataSet.DoGetFieldValue(Field: TField; Index: Integer;
  var Value: Variant);
begin
...
procedure TCustomSpringVirtualDataSet.DoOnNewRecord;
begin
...
procedure TCustomSpringVirtualDataSet.DoPostRecord(Index: Integer;
...
procedure TCustomSpringVirtualDataSet.FreeRecordBuffer(var Buffer: TRecordBuffer);
begin
...
{$IFDEF NEXTGEN}
procedure TCustomSpringVirtualDataSet.FreeRecBuf(var Buffer: TRecBuf);
begin
  FreeRecordBuffer(Buffer);
end;
{$ENDIF}

function TCustomSpringVirtualDataSet.GetActiveRecBuf(var RecBuf: TRecordBuffer): Boolean;
begin
  Pointer(RecBuf) := nil;
  case State of
...
function TCustomSpringVirtualDataSet.GetBlobFieldData(FieldNo: Integer;
...
{$IFDEF DELPHIXE4_UP}
procedure TCustomSpringVirtualDataSet.GetBookmarkData(Buffer: TRecBuf; Data: TBookmark);
begin
  PObject(Data)^ := fIndexList[PArrayRecInfo(Buffer).Index];
end;

procedure TCustomSpringVirtualDataSet.SetBookmarkData(Buffer: TRecBuf; Data: TBookmark);
begin
...
{$IFNDEF NEXTGEN}
{$IFDEF DELPHIXE3_UP}
procedure TCustomSpringVirtualDataSet.GetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
  PObject(Data)^ := fIndexList[PArrayRecInfo(Buffer).Index];
end;

procedure TCustomSpringVirtualDataSet.SetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
  if PArrayRecInfo(Buffer).BookmarkFlag in [bfCurrent, bfInserted] then
    PArrayRecInfo(Buffer).Index := fIndexList.IndexOf(PObject(@Data[0])^)
  else
...
procedure TCustomSpringVirtualDataSet.GetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  PObject(Data)^ := fIndexList[PArrayRecInfo(Buffer).Index];
end;

procedure TCustomSpringVirtualDataSet.SetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  if PArrayRecInfo(Buffer).BookmarkFlag in [bfCurrent, bfInserted] then
...
function TCustomSpringVirtualDataSet.GetBookmarkFlag(Buffer: TRecordBuffer): TBookmarkFlag;
begin
  Result := PArrayRecInfo(Buffer).BookmarkFlag;
end;

function TCustomSpringVirtualDataSet.GetCanModify: Boolean;
begin
  Result := not fReadOnly;
end;

function TCustomSpringVirtualDataSet.GetFieldData(Field: TField;
...
function TCustomSpringVirtualDataSet.GetIndex: Integer;
var
  recBuf: TRecordBuffer;
begin
  Result := -1;
...
function TCustomSpringVirtualDataSet.GetRecNo: Integer;
var
  recBuf: TRecordBuffer;
begin
  CheckActive;
...
function TCustomSpringVirtualDataSet.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode;
...
function TCustomSpringVirtualDataSet.GetRecordCount: Integer;
begin
  Result := -1;
...
function TCustomSpringVirtualDataSet.GetRecordSize: Word;
begin
  Result := SizeOf(TArrayRecInfo);
end;

procedure TCustomSpringVirtualDataSet.InternalAddRecord(
  Buffer: {$IFDEF DELPHIXE3_UP}TRecordBuffer{$ELSE}Pointer{$ENDIF}; Append: Boolean);
begin
  DoPostRecord(Current, Append);
end;

procedure TCustomSpringVirtualDataSet.InternalClose;
begin
  fInternalOpen := False;
  BindFields(False);
...
procedure TCustomSpringVirtualDataSet.InternalCreateFields;
begin
...
procedure TCustomSpringVirtualDataSet.InternalDelete;
var
  recBuf: TRecordBuffer;
begin
...
procedure TCustomSpringVirtualDataSet.InternalEdit;
begin
  fModifiedFields.Clear;
...
procedure TCustomSpringVirtualDataSet.InternalFirst;
begin
  fCurrent := -1;
end;

function TCustomSpringVirtualDataSet.InternalGetRecord(Buffer: TRecordBuffer;
...
procedure TCustomSpringVirtualDataSet.InternalGotoBookmark(
  Bookmark: {$IFDEF DELPHIXE3_UP}TBookmark{$ELSE}Pointer{$ENDIF});
begin
  fCurrent := fIndexList.IndexOf(PObject(Bookmark)^);
end;

procedure TCustomSpringVirtualDataSet.InternalHandleException;
begin
  if Assigned(Classes.ApplicationHandleException) then
    Classes.ApplicationHandleException(Self);
end;

procedure TCustomSpringVirtualDataSet.InternalInitFieldDefs;
...
procedure TCustomSpringVirtualDataSet.InternalInitRecord(Buffer: TRecordBuffer);
var
  i: Integer;
begin
  for i := 0 to Fields.Count - 1 do
...
procedure TCustomSpringVirtualDataSet.InternalLast;
begin
  fCurrent := RecordCount;
end;

procedure TCustomSpringVirtualDataSet.InternalOpen;
begin
  fInternalOpen := True;
  fCurrent := -1;
...
procedure TCustomSpringVirtualDataSet.InternalPost;
var
  recBuf: TRecordBuffer;
begin
...
procedure TCustomSpringVirtualDataSet.InternalRefresh;
begin
  fIndexList.Rebuild;
end;

procedure TCustomSpringVirtualDataSet.InternalSetToRecord(Buffer: TRecordBuffer);
begin
  if PArrayRecInfo(Buffer).BookmarkFlag in [bfCurrent, bfInserted] then
    fCurrent := PArrayRecInfo(Buffer).Index;
end;

function TCustomSpringVirtualDataSet.IsCursorOpen: Boolean;
begin
  Result := fInternalOpen;
end;

function TCustomSpringVirtualDataSet.IsFiltered: Boolean;
begin
  Result := Filtered and ((Trim(Filter) <> '') or Assigned(OnFilterRecord));
end;

function TCustomSpringVirtualDataSet.Locate(const KeyFields: string; const KeyValues: Variant;
...
function TCustomSpringVirtualDataSet.Lookup(const KeyFields: string; const KeyValues: Variant;
...
procedure TCustomSpringVirtualDataSet.SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag);
begin
  PArrayRecInfo(Buffer).BookmarkFlag := Value;
end;

procedure TCustomSpringVirtualDataSet.SetCurrent(value: Integer);
begin
  fCurrent := value;
end;

procedure TCustomSpringVirtualDataSet.SetFieldData(Field: TField; Buffer: TValueBuffer);
begin
  SetFieldData(Field, Buffer, True);
end;

procedure TCustomSpringVirtualDataSet.SetFieldData(Field: TField; Buffer: TValueBuffer; NativeFormat: Boolean);
...
procedure TCustomSpringVirtualDataSet.SetFieldData(const field: TField;
...
procedure TCustomSpringVirtualDataSet.SetFiltered(Value: Boolean);
begin
...
procedure TCustomSpringVirtualDataSet.SetIndex(const value: Integer);
begin
...
procedure TCustomSpringVirtualDataSet.SetRecBufSize;
begin
  fRowBufSize := SizeOf(TArrayRecInfo) + (Fields.Count * SizeOf(Variant));
end;

procedure TCustomSpringVirtualDataSet.SetRecNo(Value: Integer);
begin
  CheckBrowseMode;
...
constructor TCustomSpringVirtualDataSet.TBlobStream.Create(Field: TBlobField;
  Mode: TBlobStreamMode);
begin
  inherited Create;
  fField := Field;
  fFieldNo := fField.FieldNo - 1;
  fDataSet := fField.Dataset as TCustomSpringVirtualDataSet;
  fFieldData := Null;
  fData := Null;
  if Mode <> bmRead then
  begin
...
destructor TCustomSpringVirtualDataSet.TBlobStream.Destroy;
begin
  if fModified then
...
procedure TCustomSpringVirtualDataSet.TBlobStream.ReadBlobData;
var
  buffer: TBytes;
  recBuf: TRecordBuffer;
begin
...
function TCustomSpringVirtualDataSet.TBlobStream.Realloc(
...
procedure TCustomSpringVirtualDataSet.TBlobStream.Truncate;
begin
  Clear;
  fModified := True;
end;

function TCustomSpringVirtualDataSet.TBlobStream.Write(const Buffer;
