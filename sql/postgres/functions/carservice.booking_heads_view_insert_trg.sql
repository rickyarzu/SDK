...
begin
  -- un nuovo record ha id nullo o 0 quindi 'stacco' una sequence
   ...
   new.insert_date := now(); 
	...
	VALUES
		  (new.db_schema_id, new.jguid, NEW.customer_id, NEW.cust_first_name, NEW.cust_last_name, NEW.full_address,  NEW.cust_cellular, 
		   NEW.cust_email, 28, new.office_id); 
			 
	  end if; 

	INSERT INTO carservice.booking_heads(id, office_id, branch_id , jguid, db_schema_id, user_insert, insert_date, customer_id, state_id)
	VALUES (new.id, new.office_id /*ufficio o sede contabile*/, new.branch_id /*location di riferimento service*/,  
				new.jguid, new.db_schema_id, new.user_insert,  new.insert_date, new.customer_id /*id utente del servizio */, NEW.state_id);
	
		/*Se il booking è stato annullato elimino le prenotazioni fatte*/
