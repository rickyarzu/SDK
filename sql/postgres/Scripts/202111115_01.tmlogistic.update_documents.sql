...
Updates WorkIDs on AWB from Master and House AWB using Join on jguid field

----------------------------------------------------------------------------------------------------------- */

-- aerea lta house 
update logistic.awb k set work_id = 
(select id from 
(
select ah.jguid, wk.id
from 
custom_tm_logistic.aerealtahouse ah
inner join custom_tm_logistic.testatapratiche tp
  on tp.anno = ah.anno and tp.progressivo = ah.progressivo and tp.progressivopraticagroupage = ah.progressivopraticagroupage
  and tp.codattivita = ah.codattivita and tp.codfiliale = ah.codfiliale
inner join workflow.workflows wk on
   tp.jguid = wk.jguid
) a
where
a.jguid = k.jguid
